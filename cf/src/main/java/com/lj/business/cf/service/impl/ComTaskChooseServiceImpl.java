...
public class ComTaskChooseServiceImpl implements IComTaskChooseService {
	...
	public List<FindComTaskChooseIndexReturn> findComTaskChooseIndex(
		...
		try {
			...
			//luoshuming 
			if("DIS".equals(findComTaskChooseIndex.getFlag())){
				   //获取当前月第一天：
				...
				findComTaskChooseIndex.setWorkDateStart(c.getTime());
				//获取昨天
				Date date=DateUtils.getPreday(org.apache.commons.lang.time.DateUtils.truncate(new Date(), Calendar.DAY_OF_MONTH) );
				c.setTime(date);
				c.set(Calendar.HOUR_OF_DAY, 23);  
				c.set(Calendar.MINUTE, 59);  
				c.set(Calendar.SECOND,59);  
				Date endTime= c.getTime();
				findComTaskChooseIndex.setWorkDateEnd(endTime);
			}
