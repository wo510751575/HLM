...
//		AssertUtils.notNullAndEmpty(findShopTaskSetList.getMemberNoSp(), "店长编号不能为空");
		...
		try {
			...
			if (listMerchant != null && listMerchant.size() > 0) {
				...
				for (TaskSetShop taskSetShop : listMerchant) {
					...
					calendar.setTime(DateUtils.getMonthLastDay());
					...
					findShopTaskSetListReturn.setEndDate(calendar.getTime());
	...
	public List<TaskSetShopDetailReturn> toShopTaskSet(ToShopTaskSet toShopTaskSet) throws TsfaServiceException {
		...
		try {
			...
			findGuidMemberDto.setStatus(MemberStatus.NORMAL.toString());
			...
			if (listDetail == null || listDetail.size() == 0) {
				...
				for (TaskSetShopDetail taskSetShopDetail : listDetail) {
					...
					boolean flag = false;//当导购离职后，需导购表和店长设置任务明细表中同时存在才添加任务
	...
	public void shopTaskSet(ShopTaskSet shopTaskSet) throws TsfaServiceException {
		logger.debug("shopTaskSet(ShopTaskSet shopTaskSet={}) - start", shopTaskSet); //$NON-NLS-1$
		...
		try {
			...
			for (ShopTaskSetDetailList shopTaskSetDetailList : list) {
				...
				if(shopTaskSetDetailList.getNumDay() > Integer.MAX_VALUE){
					throw new TsfaServiceException(ErrorCode.TASK_SET_SHOP_NUM_OVERSIZE, "店长设置任务数据过大！");
				}
				
				// code为空则添加任务设置明细
				if (StringUtils.isEmpty(shopTaskSetDetailList.getCode())) {
					logger.debug("进入新增方法！--------------------------！！！！！！！！！！！！！！！"+shopTaskSetDetailList);
				...
				findWorkTask.setMemberNoGm(shopTaskSetDetailList.getMemberNoGm());
				findWorkTask.setWorkDate(new Date());
				findWorkTask.setTaskType(shopTaskSet.getTaskType());
				FindWorkTaskReturn findWorkTaskReturn = workTaskService.findWorkTaskByGm(findWorkTask);
				// luoshuming 判断是否有当天工作任务,没有则新增,有则修改
		        if(findWorkTaskReturn !=null){
					UpdateWorkTask updateWorkTask = new UpdateWorkTask();
					updateWorkTask.setCode(findWorkTaskReturn.getCode());
					updateWorkTask.setRequireNum(shopTaskSetDetailList.getNumDay());
					updateWorkTask.setUnfinishNum(shopTaskSetDetailList.getNumDay() - findWorkTaskReturn.getFinishNum());
					workTaskService.updateWorkTask(updateWorkTask);
		        }else{
		        	FindWorkTaskList findWorkTaskList = new FindWorkTaskList();
		        	findWorkTaskList.setTaskType(shopTaskSet.getTaskType());
		        	...
		        	addWorkTask.setMerchantNo(shopTaskSet.getMerchantNo());
		        	addWorkTask.setMemberNoGm(shopTaskSet.getMemberNoSp());
		        	addWorkTask.setMemberNameGm(shopTaskSet.getMemberNameSp());
		        	addWorkTask.setShopNo(shopTaskSet.getShopNo());
		        	addWorkTask.setShopName(shopTaskSet.getShopName());
		        	addWorkTask.setCodeList(findWorkTaskListReturn.getCode());
		        	addWorkTask.setNameList(findWorkTaskListReturn.getTaskName());
		        	addWorkTask.setTaskType(findWorkTaskListReturn.getTaskType());
		        	addWorkTask.setWorkDate(DateUtils.getPreday(new Date()));
					addWorkTask.setRequireNum(0L);
					if(shopTaskSetDetailList.getNumDay() > 0){
						addWorkTask.setRequireNum(shopTaskSetDetailList.getNumDay());
					}else{
						addWorkTask.setRequireNum(0L);
					}
					addWorkTask.setTaskUnit(findWorkTaskListReturn.getTaskUnit());
				    //暂不查询未设置当天工作任务前完成的任务数量或金额
	...
	public MangerWorkReturn findMangerWork(FindMangerWork findMangerWork) throws TsfaServiceException {
		...
		try {
			...
			if (list != null && list.size() > 0) {
				...
				mangerWorkReturn.setAvgWorkScore(big.setScale(1, BigDecimal.ROUND_HALF_UP).intValue());
			}
