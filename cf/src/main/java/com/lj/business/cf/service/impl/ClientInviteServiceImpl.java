...
public class ClientInviteServiceImpl implements IClientInviteService { 
	...
	public void addInviteRecord(FindAddInviteRecordDto findAddInviteRecordDto) throws TsfaServiceException {
		...
		try{
			...
			if("Y".equals(findAddInviteRecordDto.getInviteResult())){
				...
				addClientFollow.setDis(findAddInviteRecordDto.getDis());
				addClientFollow.setComTaskCode(findAddInviteRecordDto.getComTaskCode());//原业务任务CODE
				//查询到店体验的code
				FindComTaskList findComTaskList = new FindComTaskList();
				...
				generateNextDate.setMemberNo(findAddInviteRecordDto.getMemberNo());
				generateNextDate.setMemberNoGm(findAddInviteRecordDto.getMemberNoGm());
	...
	public List<Map<String, Object>> findInviteFinish(
		...
		logger.debug("findInviteFinish(Map<String, Object> map={}) - start", map); //$NON-NLS-1$
		return clientInviteDao.findInviteFinish(map);
	}
	...
	public List<Map<String, Object>> findPromiseInvite(
			Map<String, Object> map) {
		logger.debug("findInviteFinish(Map<String, Object> map={}) - start", map); //$NON-NLS-1$
		return clientInviteDao.findPromiseInvite(map);
	}

	@Override
	public FindClientInviteReturn findClientInvinteReachTime(
			FindClientInvite clientInvite) {
		logger.debug(" findClientInviteMemberNo(FindClientInvite clientInvite)", clientInvite); 
		AssertUtils.notAllNull(clientInvite.getMemberNoGm(),"导购编号不能为空");
		AssertUtils.notAllNull(clientInvite.getMemberNo(),"客户编号不能为空");
		FindClientInviteReturn inviteReturn ;
		try {
			inviteReturn =clientInviteDao.findClientInvinteReachTime(clientInvite);
		} catch (Exception e) {
			logger.error("查找客户邀约表信息信息错误！",e);
			throw new TsfaServiceException(ErrorCode.CLIENT_INVITE_FIND_ERROR,"查找客户邀约表信息信息错误！",e);
		}
	...
	@Override
	public int  findClientInviteMemberNo(
			FindClientInvite clientInvite)throws TsfaServiceException {
		logger.debug(" findClientInviteMemberNo(FindClientInvite clientInvite)", clientInvite); 
		AssertUtils.notAllNull(clientInvite.getMemberNoGm(),"导购编号不能为空！");
		//List<FindClientInviteReturn> list = null;
/*		int num =0;
		int memberNum=0;*/
		try {
			/*		list=clientInviteDao.findClientInviteMemberNo(clientInvite);
			if(list.size()>0){
				ClientExpDto clientExpDto=new ClientExpDto(); 
				for(FindClientInviteReturn inviteReturn:list){
					clientExpDto.setMemberNoGm(clientInvite.getMemberNoGm());
					clientExpDto.setMemberNo(inviteReturn.getMemberNo());
					FindClientExpReturn clientExpReturns=clientExpService.findClientExpTodayNum(clientExpDto);
				    if(clientExpReturns !=null){
				    	num++;
				    }
				}
				
				memberNum=list.size()-num;
			}
			return memberNum;*/
		return 1;
		} catch (Exception e) {
