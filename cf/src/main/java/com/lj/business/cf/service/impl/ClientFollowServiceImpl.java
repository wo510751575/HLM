...
import com.lj.business.cf.emus.PmTypeFlag;
...
public class ClientFollowServiceImpl implements IClientFollowService { 
	...
	public AddClientFollowReturn addCFOrder(AddClientFollow addClientFollow,String flag,boolean isCreateCf,boolean isAddIntegal) throws TsfaServiceException {
		...
		if(PmTypeFlag.NO_SUCCESS.getCode().equals(flag)){
		...
		try {
			...
			}else if(PmTypeFlag.SUCCESS.getCode().equals(flag)){//成单
				successFlag(addClientFollow, clientFollow);
			}else if(PmTypeFlag.GIVE_UP.getCode().equals(flag)){//放弃
				giveupFlag(addClientFollow, cfNo, clientFollow);
			}
			...
			//总表备注同步为当前跟踪详情的备注 2018.04.16 hy from 宇翠 需求
			updateClientFollowSummary.setRemark(addClientFollow.getRemark()); 
			updateClientFollowSummary.setRemark2(addClientFollow.getRemark2());
			//总表备注同步end
			...
			//update by LeoPeng 2017-10-30 添加不为NULL的判断
			if(findGuidMemberReturn != null){
	 ...
	 * @param clientFollow
	 */
	public void giveupFlag(AddClientFollow addClientFollow, String cfNo,
			ClientFollow clientFollow) {
		//没有店长不能暂停跟进客户
		FindGuidMember findGuidMember = new FindGuidMember();
		findGuidMember.setMemberNo(addClientFollow.getMemberNoGm());
		...
		findManagerMemberPage.setMemberNoShop(guidMember.getShopNo());
		List<FindManagerMemberPageReturn> returnShopNo=managerMemberService.findMemberNoShop(findManagerMemberPage);
		if(returnShopNo.size() <= 0){
			throw new TsfaServiceException(ErrorCode.PM_ABANDON_CHECK_SHOP_ERROR,"门店不存在店长,不能进行暂停客户！");
		}
		...
		if(cfCount < ac){
			throw new TsfaServiceException(ErrorCode.PM_ABANDON_CHECK_TIME_ERROR,"客户暂停需要跟进3次以上！");
		}
	}
	 ...
	 * @param clientFollow
	 */
	public void successFlag(AddClientFollow addClientFollow,
			ClientFollow clientFollow) {
		clientFollow.setComTaskType(ComTaskType.SYSTEM.toString());
		AssertUtils.notNullAndEmpty(addClientFollow.getOrderAmount(), "成单金额不能为空");
		addClientFollow.setFollowType(FollowType.SYSTEM.toString());
		
		Date deliverTime = addClientFollow.getDeliverTime();
		String formatDate = "";
		if (deliverTime != null) {
			formatDate = ",预计送货时间" + DateUtils.formatDate(deliverTime, DateUtils.YYYY年MM月DD日);
		}
		
		addClientFollow.setFollowInfo("成单结束跟进" + formatDate);
		...
		findWorkTask.setMemberNoGm(addClientFollow.getMemberNoGm());
		findWorkTask.setWorkDate(new Date());
		findWorkTask.setTaskType(TaskType.XIAO_SHOU.toString());
		FindWorkTaskReturn findWorkTaskReturn = workTaskService.findWorkTaskByGm(findWorkTask);
		
		UpdateWorkTask updateWorkTask = new UpdateWorkTask();
		updateWorkTask.setCode(findWorkTaskReturn.getCode());
		updateWorkTask.setFinishNum(findWorkTaskReturn.getFinishNum() + addClientFollow.getOrderAmount());
		long unFinish = findWorkTaskReturn.getUnfinishNum() - addClientFollow.getOrderAmount();
		updateWorkTask.setUnfinishNum(unFinish < 0 ? 0 : unFinish);
		workTaskService.updateWorkTask(updateWorkTask);
		logger.debug(updateWorkTask+""+updateWorkTask.getFinishNum());
	}
	...
	private void cfAbandonProcess(AddClientFollow addClientFollow,
		...
		if(frList != null && frList.size() > 0){
			throw new TsfaServiceException(ErrorCode.PM_ABANDON_NOCHECK_DATA_ERROR,"存在未审批的暂停记录,不能暂停！");
		}
		...
		addMessageInfo.setMerchantNo(merchantNo);
		addMessageInfo.setAbandonCode(addPmAbandonReturn.getCode());
		if(findManagerMemberPageReturn!=null){
			addMessageInfo.setMemberName(findManagerMemberPageReturn.getMemberName());
			addMessageInfo.setMemberNo(findManagerMemberPageReturn.getMemberNo());
		}
		addMessageInfo.setMemberNameSender(memberNameGm);
		addMessageInfo.setMemberNoSender(memberNoGm);
		addMessageInfo.setTitle("暂停跟进客户{memberName}的申请".replace("{memberName}", memberName));
		addMessageInfo.setContent(addClientFollow.getResean());

		logger.info("=====addMessageInfo:" + addMessageInfo);

		messageInfoService.addMessageInfo(addMessageInfo);
		//添加审核消息	段志鹏	2017-08-03 end
			...
			if(addClientFollow.getNextDate() == null){
				...
				if(findComTaskChooseByCodeReturnDto != null){
					if(ComTaskType.INVITE.toString().equals(findComTaskChooseByCodeReturnDto.getComTaskType())){
						...
						generateNextDate.setMemberNo(addClientFollow.getMemberNo());
						generateNextDate.setMemberNoGm(addClientFollow.getMemberNoGm());
	...
	private void cfProcess(AddClientFollow addClientFollow,
		...
		addComTask.setOrgComTaskCode(addClientFollow.getComTaskCode());//设置 原客户沟通任务Code 
	...
	/*	UpdateWorkTaskFinishNum updateWorkTaskFinishNum = new UpdateWorkTaskFinishNum();
		...
		Date addNow = org.apache.commons.lang.time.DateUtils.truncate(new Date(), Calendar.DAY_OF_MONTH);
		addSalesGmDayDetail.setMerchantNo(addClientFollow.getMerchantNo());
		addSalesGmDayDetail.setAreaCode(addClientFollow.getAreaCode());
		addSalesGmDayDetail.setAreaName(addClientFollow.getAreaName());
		addSalesGmDayDetail.setShopNo(addClientFollow.getShopNo());
		addSalesGmDayDetail.setShopName(addClientFollow.getShopName());
		addSalesGmDayDetail.setHeadAddress(addClientFollow.getHeadAddress());
		addSalesGmDayDetail.setDaySt(addNow);
		addSalesGmDayDetail.setMemberNoGm(addClientFollow.getMemberNoGm());
		addSalesGmDayDetail.setMemberNameGm(addClientFollow.getMemberNameGm());
		addSalesGmDayDetail.setNumSale(addClientFollow.getOrderAmount());
		//查询销售目标
		FindTaskSetShopDetail findTaskSetShopDetail = new FindTaskSetShopDetail();
		findTaskSetShopDetail.setMerchantNo(addClientFollow.getMerchantNo());
		findTaskSetShopDetail.setMemberNoGm(addClientFollow.getMemberNoGm());
		findTaskSetShopDetail.setShopNo(addClientFollow.getShopNo());
		findTaskSetShopDetail.setNow(addNow);
		...
		if(findTaskSetShopDetailReturn != null){
			addSalesGmDayDetail.setNumSaleTarget(findTaskSetShopDetailReturn.getNumDay());
		}else{
			addSalesGmDayDetail.setNumSaleTarget(0L);
		}
		salesGmDayDetailService.addOrUpdateSalesGmDayDetail(addSalesGmDayDetail);
		...
		updateClientFollowSummary2.setDeliverTime(addClientFollow.getDeliverTime());
		updateClientFollowSummary2.setEndDate(new Date());
		updateClientFollowSummary2.setStatus(Status.DEAL.toString());
		if (StringUtils.isNotEmpty(personMember.getBomName())) {//设置购买产品名称
			updateClientFollowSummary2.setBomName(personMember.getBomName());
		}
		...
		/*if(findPmTypeReturn == null || StringUtils.isEmpty(findPmTypeReturn.getFreValue())){
			hour = Integer.valueOf(PublicConstants.FRE_VALUE);
		}else{
	...
	public AbandonCheckReturn abandonCheck(AbandonCheckDto abandonCheckDto) throws TsfaServiceException {
		...
		try {
			...
			if(findClientFollowSummaryReturn == null 
					|| findClientFollowSummaryReturn.getEndDate() != null){
				throw new TsfaServiceException(ErrorCode.NOT_MANAGER_MEMBER_DATA_ERROR,"跟进数据错误！");
			}
	...
	public List<Map<String, Object>> findSumFollowGroupByShop(
		...
		logger.debug("findSumFollowGroupByShop(Map<String, Object> map={}) - start", map);
		...
		for (FindShopPageReturn findShopPageReturn : shops) {
			...
			if(guidMemberList.size() > 0){
				...
				Integer numPmUngroup = clientFollowDao.findSumFollowShop(map);
				shopData.put("numPmUngroup", numPmUngroup);
			}else
				shopData.put("numPmUngroup", 0);
	...
	public ClientFollow findClientFollowSelect(FindClientFollow findClientFollow) {
		logger.debug("findClientFollowSelect(FindClientFollow findClientFollow={}) - start", findClientFollow);
		return clientFollowDao.findClientFollowSelect(findClientFollow);
	}


	@Override
	public Long findCountNumPmCfDayByGm(Map<String, Object> map) {
		logger.debug("findCountNumCfDayByGm(Map<String, Object> map={}) - start", map);
		return clientFollowDao.findCountNumPmCfDayByGm(map);
	}
	...
	public Long findCountNumCfDayByGm(Map<String, Object> map) {
		logger.debug("findCountNumCfDayByGm(Map<String, Object> map={}) - start", map);
		return clientFollowDao.findCountNumCfDayByGm(map);
	}


	@Override
	public List<Map<String, Object>> getClientFollowInfo(Map<String, Object> map) {
		logger.debug("getClientFollowInfo(Map<String, Object> map)={}) - start", map); //$NON-NLS-1$

		AssertUtils.notNull(map);
		List<Map<String, Object>> list = null;
		try{
			list = clientFollowDao.getClientFollowInfo(map);
		}catch (Exception e) {
			logger.error("跟进记录查询错误",e);
			throw new TsfaServiceException(ErrorCode.CLIENT_FOLLOW_KEEP_FIND_ERROR,"跟进记录查询错误！",e);
		}
		logger.debug("getClientFollowInfo(Map<String, Object>) - end - return value={}", list); //$NON-NLS-1$
		return list;
	}
