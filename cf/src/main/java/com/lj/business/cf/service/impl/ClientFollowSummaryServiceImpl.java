...
public class ClientFollowSummaryServiceImpl implements IClientFollowSummaryService { 
	...
	public String addClientFollowSummary(
		...
		try {
			...
			clientFollowSummary.setBomCode(addClientFollowSummary.getBomCode());
			clientFollowSummary.setBomName(addClientFollowSummary.getBomName());
			...
			clientFollowSummary.setDeliverTime(addClientFollowSummary.getDeliverTime());
	...
	public void updateClientFollowSummary(
		...
		try {
			...
			clientFollowSummary.setDeliverTime(updateClientFollowSummary.getDeliverTime());
	...
	public void updateClientFollowSummaryByCfNo(
		...
		AssertUtils.notNull(updateClientFollowSummary);
		if (StringUtils.isEmpty(updateClientFollowSummary.getCfNo())) {
			throw new TsfaServiceException(ErrorCode.CLIENT_FOLLOW_NUMBER_IS_EMPTY,"跟进编号为空！");
		}
		try {
			ClientFollowSummary clientFollowSummary = new ClientFollowSummary();
			//update数据录入
			...
			clientFollowSummary.setMemberNameGm(updateClientFollowSummary.getMemberNameGm());
			clientFollowSummary.setCfNo(updateClientFollowSummary.getCfNo());
			clientFollowSummary.setBomName(updateClientFollowSummary.getBomName());
			clientFollowSummary.setFollowNum(updateClientFollowSummary.getFollowNum());
			clientFollowSummary.setStatus(updateClientFollowSummary.getStatus());
			clientFollowSummary.setOrderAmount(updateClientFollowSummary.getOrderAmount());
			clientFollowSummary.setDeliverTime(updateClientFollowSummary.getDeliverTime());
	...
	public FindClientFollowSummaryReturn findClientFollowSummary(
		...
		try {
			...
			findClientFollowSummaryReturn.setDeliverTime(clientFollowSummary.getDeliverTime());
	...
	public String addSummaryWithClientFollow(AddPmClientFollowFirstDto addPmClientFollowFirstDto) throws TsfaServiceException {
		...
		try{
			...
			addClientFollowSummary.setBomName(addPmClientFollowFirstDto.getBomName());
	...
	@Override
	public int findCountPmOrderByGm(Map<String, Object> map) {
		logger.debug("findCountPmOrderByGm(Map<String, Object> map={}) - start", map);
		AssertUtils.notNull(map);
		int count= 0;
		try {
			count = clientFollowSummaryDao.findCountPmOrderByGm(map);
		} catch (Exception e) {
			logger.error("查询成单客户数量错误！",e);
			return count;
		}
		return count;
	}
	...
	@Override
	public List<Map<String, Object>> findNumSaleGroupByShop(
			Map<String, Object> map) {
		logger.debug("findNumSaleGroupByShop(Map<String, Object> map={}) - start", map);
		...
		for (FindShopPageReturn findShopPageReturn : shops) {
			...
			if(guidMemberList.size() > 0){
				...
				Double sale = clientFollowSummaryDao.findSaleByShop(map);
				shopData.put("sale", sale);
			}else
				shopData.put("sale", 0);
	...
	@Override
	public List<Map<String, Object>> findMaxSumFollow(
			Map<String, Object> map) {
		logger.debug("findMaxSumFollow(Map<String, Object> map={}) - start", map);
		FindShop findShop = new FindShop();
		findShop.setMemberNoMerchant(map.get("merchantCode").toString());
		List<FindShopPageReturn> shops = shopService.findShops(findShop);
		...
		for (FindShopPageReturn findShopPageReturn : shops) {
			...
			if(guidMemberList.size() > 0){
				...
				map.put("memberNoGms", memberNoGms);
				Integer maxPmCfNum = clientFollowSummaryDao.findMaxSumFollow(map);
				shopData.put("maxPmCfNum", maxPmCfNum);
			}else
				shopData.put("maxPmCfNum", 0);
	...
	@Override
	public List<Map<String, Object>> findCountPmOrder(
			Map<String, Object> map) {
		...
		for (FindShopPageReturn findShopPageReturn : shops) {
			...
			if(guidMemberList.size() > 0){
				...
				map.put("memberNoGms", memberNoGms);
				Integer numOrderPm = clientFollowSummaryDao.findCountPmOrder(map);
				shopData.put("numOrderPm", numOrderPm);
			}else
				shopData.put("numOrderPm", 0);
	...
	@Override
	public FindImClientFollowReturn findImClientFollow(FindClientFollowSummary summary) {
		AssertUtils.notNull(summary);
		AssertUtils.notNullAndEmpty(summary.getMerchantNo(),"商户编号不能为空！");
		AssertUtils.notNullAndEmpty(summary.getMemberNo(),"客户编号不能为空！");
		return clientFollowSummaryDao.findImClientFollow(summary);
	}
}
