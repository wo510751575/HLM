...
import com.lj.business.cf.dto.comTask.AddWorkTaskByNewGM;
import com.lj.business.cf.dto.comTask.CloseGroup;
...
import com.lj.business.cf.dto.comTask.FindLastComTaskReturn;
...
public class ComTaskServiceImpl implements IComTaskService { 
	...
	public Date generateNextDate(GenerateNextDate generateNextDate)
		...
		try {
			...
			c.setTime(nextDate);
			//update by 梅宏博   1.0.4需求在客户填写周期后，需要根据周期生产邀约任务
			
			FindPersonMemberReturn personMemberReturn = null;
			
			if (generateNextDate.getMemberNo() != null && generateNextDate.getMemberNoGm() != null) {
				FindPersonMember findPersonMember = new FindPersonMember();
				findPersonMember.setMemberNo(generateNextDate.getMemberNo());
				findPersonMember.setMemberNoGm(generateNextDate.getMemberNoGm());
				personMemberReturn = personMemberService.findPersonMemberByMGM(findPersonMember);
			}
			
			if (personMemberReturn != null && personMemberReturn.getConsumeFrequency() != null && personMemberReturn.getConsumeFrequency() >= 0) {
				
				c.add(Calendar.DATE, personMemberReturn.getConsumeFrequency());
			} else {
	...
	public Page<FindComTaskIndexPageReturn> findComTaskIndexPage(
		...
		try {
			...
			findComTaskIndexPage.setWorkDateEnd(workDateEnd);
				if(findComTaskIndexPage.getFlag().equals("DIS")){
				     Date date = org.apache.commons.lang.time.DateUtils.truncate(now, Calendar.MONTH);
				     findComTaskIndexPage.setWorkDateStart(date);
				     
				     Date endTime = DateUtils.addDays(now, 0);
					 endTime = DateUtils.addMilliseconds(endTime, -2);
				     findComTaskIndexPage.setWorkDateEnd(endTime);
				     System.out.println(endTime);
				     logger.debug("endTime:", endTime); //$NON-NLS-1$
				}
			...
			if(returnList.size() != 0){
				...
				for (FindComTaskIndexPageReturn findComTaskIndexPageReturn : returnList) {
					...
					//添加到店时间   1.04版本需求加字段
					clientInvite.setMemberNoGm(findComTaskIndexPage.getMemberNoGm());
					clientInvite.setMemberNo(findComTaskIndexPageReturn.getMemberNo());
					FindClientInviteReturn clientInviteReturn= clientInviteService.findClientInvinteReachTime(clientInvite);
					if(clientInviteReturn !=null){
						SimpleDateFormat sdf = new SimpleDateFormat("yyyy年MM月dd日 HH时mm分");
						String reachTime = sdf.format(clientInviteReturn.getReachTime());
						findComTaskIndexPageReturn.setReachTime(reachTime);
					}
				}
				...
				Map<String, FindPmWxInfoReturn> findPmWxInfoReturnMap = new HashMap<String, FindPmWxInfoReturn>();
				for (FindPmWxInfoReturn findPmWxInfoReturn : list) {	
					findPmWxInfoReturnMap.put(findPmWxInfoReturn.getMemberNo(), findPmWxInfoReturn);
				}
				for (FindComTaskIndexPageReturn findComTaskIndexPageReturn : returnList) {
					FindPmWxInfoReturn findPmWxInfoReturn = findPmWxInfoReturnMap.get(findComTaskIndexPageReturn.getMemberNo());
					if(findPmWxInfoReturn != null){
						...
						findComTaskIndexPageReturn.setSex(findPmWxInfoReturn.getSex());
					}
				}
			}
	 ...
	 * @param requireNum the require num
	 */
	...
	/* (non-Javadoc)
	 * @see com.lj.business.cf.service.IComTaskService#findExpResults(java.util.Map)
	 */
	@Override
	...
	/* (non-Javadoc)
	 * @see com.lj.business.cf.service.IComTaskService#findCountFinishByDay(com.lj.business.cf.dto.comTask.FindComTask)
	 */
	@Override
	...
	/* (non-Javadoc)
	 * @see com.lj.business.cf.service.IComTaskService#initWorkTaskByNewGM(com.lj.business.cf.dto.comTask.AddWorkTaskByNewGM)
	 */
	@Override
	public void initWorkTaskByNewGM(AddWorkTaskByNewGM addWorkTaskByNewGM) throws TsfaServiceException {
		logger.debug("initWorkTaskByNewGM(AddWorkTaskByNewGM addWorkTaskByNewGM)={}) - start", addWorkTaskByNewGM); //$NON-NLS-1$
		AssertUtils.notNull(addWorkTaskByNewGM);
		AssertUtils.notNullAndEmpty(addWorkTaskByNewGM.getMerchantNo(), "商户编号不能为空");
		AssertUtils.notNullAndEmpty(addWorkTaskByNewGM.getShopNo(), "分店不能为空");
		AssertUtils.notNullAndEmpty(addWorkTaskByNewGM.getMemberNoGm(), "导购编号不能为空");
		
		// 查询商户设置的工作事项
		List<String> merchantNoList = new ArrayList<String>();
		merchantNoList.add(addWorkTaskByNewGM.getMerchantNo());
		List<FindWorkTaskChoosePageDto> merchantTaskChooseList = workTaskChooseService.findWorkTaskChooseByMerchantNo(merchantNoList);	
		Date now = new Date();
		if(!CollectionUtils.isEmpty(merchantTaskChooseList)) {
			// 为导购新增商户设置的每一项工作事项
			for(FindWorkTaskChoosePageDto merchantTaskChoose : merchantTaskChooseList) {
				AddWorkTask addWorkTask = new AddWorkTask();
				addWorkTask.setMerchantNo(addWorkTaskByNewGM.getMerchantNo());
				addWorkTask.setShopNo(addWorkTaskByNewGM.getShopNo());
				addWorkTask.setShopName(addWorkTaskByNewGM.getShopName());
				addWorkTask.setMemberNoGm(addWorkTaskByNewGM.getMemberNoGm());
				addWorkTask.setMemberNameGm(addWorkTaskByNewGM.getMemberNameGm());
				addWorkTask.setCodeList(merchantTaskChoose.getCodeList());
				addWorkTask.setNameList(merchantTaskChoose.getNameList());
				addWorkTask.setWorkDate(now);
				addWorkTask.setRequireNum(0L);
				addWorkTask.setTaskUnit(merchantTaskChoose.getTaskUnit());
				addWorkTask.setTaskType(merchantTaskChoose.getTaskType());
			...
			logger.warn(addWorkTaskByNewGM.getMerchantNo() + "商户没有设置工作事项");
		}
		
		logger.debug("initWorkTaskByNewGM(AddWorkTaskByNewGM addWorkTaskByNewGM)={}) - end"); //$NON-NLS-1$
	}


	/* (non-Javadoc)
	 * @see com.lj.business.cf.service.IComTaskService#findComTaskProduce(java.util.Map)
	 */
	@Override
	public List<Map<String, Object>> findComTaskProduce(
			Map<String, Object> map) {
		logger.debug("findComTaskProduce(Map<String, Object> map={}) - start", map);
		return comTaskDao.findComTaskProduceByShop(map);
	}


	/* (non-Javadoc)
	 * @see com.lj.business.cf.service.IComTaskService#findComTaskFinish(java.util.Map)
	 */
	@Override
	public List<Map<String, Object>> findComTaskFinish(
			Map<String, Object> map) {
		logger.debug("findComTaskFinish(Map<String, Object> map={}) - start", map);
		return comTaskDao.findComTaskFinish(map);
	}


	/* (non-Javadoc)
	 * @see com.lj.business.cf.service.IComTaskService#findLastRecordByMGM(com.lj.business.cf.dto.comTask.FindComTask)
	 */
	@Override
	public FindComTaskReturn findLastRecordByMGM(FindComTask findComTask) {
		logger.debug("findLastRecordByMGM(FindComTask findComTask={}) - start", findComTask);
		return comTaskDao.findLastRecordByMGM(findComTask);
	}


	/* (non-Javadoc)
	 * @see com.lj.business.cf.service.IComTaskService#queryComTaskFinish(com.lj.business.cf.dto.comTask.FindComTask)
	 */
	@Override
	public List<FindComTaskReturn> queryComTaskFinish(
			FindComTask findComTask) {
		logger.debug("queryComTaskFinish(FindComTask findComTask={}) - start", findComTask);
		AssertUtils.notNullAndEmpty(findComTask.getMerchantNo(), "商户编号不能为空");
		List<FindComTaskReturn> list= null;
		try { 
			list=comTaskDao.queryComTaskFinish(findComTask);
		} catch (Exception e) {
			logger.error("客户沟通任务表信息查询错误",e);
			throw new TsfaServiceException(ErrorCode.COM_TASK_FIND_PAGE_ERROR,"客户沟通任务表信息查询错误.！",e);
		}
	...
	/* (non-Javadoc)
	 * @see com.lj.business.cf.service.IComTaskService#closeGroup(com.lj.business.cf.dto.comTask.CloseGroup)
	 */
	@Override
	public void closeGroup(CloseGroup closeGroup) {
		logger.debug("closeGroup(CloseGroup closeGroup={}) - start", closeGroup); //$NON-NLS-1$

		AssertUtils.notNull(closeGroup);
		AssertUtils.notNullAndEmpty(closeGroup.getMerchantNo(), "商户编号不能为空");
		try{
			int count = comTaskDao.closeGroup(closeGroup);
			logger.debug("关闭分组任务，数量：" + count);
		}catch (TsfaServiceException e) {
			logger.error(e.getMessage(),e);
			throw e;
		} catch (Exception e) {
			logger.error("关闭分组任务错误",e);
			throw new TsfaServiceException(ErrorCode.COM_TASK_CLOSE_GROUP_ERROR,"关闭分组任务错误",e);
		}
		
		logger.debug("closeGroup(CloseGroup) - end"); //$NON-NLS-1$
	}


	@Override
	public List<FindLastComTaskReturn> findLastRecordByGm(
			FindComTask findComTask) {
		logger.debug("findLastRecordByGm(FindComTask findComTask={}) - start", findComTask); //$NON-NLS-1$
		List<FindLastComTaskReturn> list = comTaskDao.findLastRecordByGm(findComTask);
		logger.debug("findLastRecordByGm(List<FindLastComTaskReturn>={}) - end",list); //$NON-NLS-1$
		return list;
	}
