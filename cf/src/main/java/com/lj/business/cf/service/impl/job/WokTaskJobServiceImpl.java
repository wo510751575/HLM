	...
	private void doAddWorkTask(Collection<FindGuidMemberPageReturn> members) {
		try {
			if (!CollectionUtils.isEmpty(members)) {
				...
				for (FindGuidMemberPageReturn each : members) {
					logger.info("【工作任务统计】为每个导购增加所在商户的任务,MemberNo: " + each.getMemberNo() +" MemberName: " + each.getMemberName() + " 商户名称" + each.getMerchantName());
					...
					if (!CollectionUtils.isEmpty(merchantTaskList)) {
						for(FindWorkTaskChoosePageDto eachMerchantTask : merchantTaskList) {
							try {
								...
								// 销售目标和新增客户
								if (ShopTaskType.XIAO_SHOU.toString().equals(eachMerchantTask.getTaskType())
										|| ShopTaskType.XIN_ZENG.toString().equals(eachMerchantTask.getTaskType())
										|| ShopTaskType.MOMENTS.toString().equals(eachMerchantTask.getTaskType())) {
