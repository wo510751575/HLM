...
public class WorkTaskServiceImpl implements IWorkTaskService { 
	...
	public UpdateWorkTaskReturn updateWorkTask(
		...
		try {
			...
			workTask.setMerchantNo(updateWorkTask.getMerchantNo());
	...
	public List<FindWorkTaskMainListReturn> findWorkTaskMainList(
		...
		try {
			...
			for (FindWorkTaskMainListReturn findWorkTaskMainListReturn : returnList) {
				...
				//luoshuming 2017/10/12  ratioWork
               if(0 == findWorkTaskMainListReturn.getRequireNum()){
            	   findWorkTaskMainListReturn.setUnfinishNum(0L);
               }else{
            	   if(findWorkTaskMainListReturn.getFinishNum()<0){
            		   long unfinishNum=findWorkTaskMainListReturn.getRequireNum()-findWorkTaskMainListReturn.getFinishNum();
            		   findWorkTaskMainListReturn.setFinishNum(unfinishNum);
            	   }
            	   long unfinishNum=findWorkTaskMainListReturn.getRequireNum()-findWorkTaskMainListReturn.getFinishNum();
            	   if(unfinishNum < 0){
            		   unfinishNum=0;
            	   }
            	   findWorkTaskMainListReturn.setUnfinishNum(unfinishNum);
               }
	...
	@Override
	public Integer findUnfinishNumByType(FindWorkTask findWorkTask) {
		AssertUtils.notNull(findWorkTask);
		AssertUtils.notNullAndEmpty(findWorkTask.getMerchantNo(),"商户编号不能为空！");
		AssertUtils.notNullAndEmpty(findWorkTask.getMemberNoGm(),"导购编号不能为空！");
		AssertUtils.notNullAndEmpty(findWorkTask.getTaskType(),"任务类型不能为空！");
		return workTaskDao.findUnfinishNumByType(findWorkTask);
	}

	@Override
	public FindWorkTaskReturn findWorkTaskByGm(FindWorkTask findWorkTask) {
		logger.debug("findWorkTaskByGm(FindWorkTask findWorkTask={}) - start", findWorkTask); //$NON-NLS-1$
		AssertUtils.notNullAndEmpty(findWorkTask.getMemberNoGm(),"导购编号不能为空！");
		FindWorkTaskReturn workTask = workTaskDao.findWorkTaskByGm(findWorkTask);
		logger.debug("findWorkTaskByGm(FindWorkTaskReturn) - end - return value={}", workTask); 
		return workTask;
	}
