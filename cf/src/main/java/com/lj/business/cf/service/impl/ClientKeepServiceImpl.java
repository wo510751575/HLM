	...
	public AddClientKeepReturn newClientKeepInfo(AddClientKeep addClientKeep) throws TsfaServiceException {
		...
		try {
			...
			if(BuyType.Y.toString().equals(addClientKeep.getBuy())){
				...
				addPmClientFollowFirstDto.setBomName(addClientKeep.getBomName());
			...
			findPmInfoAll.setMemberNo(addClientKeep.getMemberNo());
			findPmInfoAll.setMemberNoGm(addClientKeep.getMemberNoGm());
			...
			if (StringUtils.isNotEmpty(findPmInfoAllReturn.getMemberName())) {
				nickNameRemarkLocal += findPmInfoAllReturn.getMemberName();
			}
			if (StringUtils.isNotEmpty(findPmInfoAllReturn.getMobile())) {
				nickNameRemarkLocal += ("-" + findPmInfoAllReturn.getMobile());
			}
			if (StringUtils.isNotEmpty(findPmInfoAllReturn.getBomName())) {
				nickNameRemarkLocal += ("-" + (StringUtils.toString(findPmInfoAllReturn.getBomName()).length()>9?findPmInfoAllReturn.getBomName().substring(0,10)+"...":StringUtils.toString(findPmInfoAllReturn.getBomName())));
			}
