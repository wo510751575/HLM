...
public class WorkTaskListServiceImplTest extends SpringTestCase{
	...
	public void addWorkTaskList() throws TsfaServiceException{
		...
		addWorkTaskList.setTaskName("发布朋友圈");
		addWorkTaskList.setTaskDesc("发布朋友圈");
		addWorkTaskList.setTaskType(ShopTaskType.MOMENTS.toString());
		addWorkTaskList.setStatus("Y");
		addWorkTaskList.setCreateId("安家管理员");
		addWorkTaskList.setTaskUnit(UnitType.GE.toString());
		addWorkTaskList.setCreateDate(new Date());
		workTaskListService.addWorkTaskList(addWorkTaskList);
	}
