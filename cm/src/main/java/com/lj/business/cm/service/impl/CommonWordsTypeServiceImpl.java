...
import com.lj.business.cm.dao.ICommonWordsTypeDao;
 ...
 */
@Service
public class CommonWordsTypeServiceImpl implements ICommonWordsTypeService { 

	
	/** Logger for this class. */
	private static final Logger logger = LoggerFactory.getLogger(CommonWordsTypeServiceImpl.class);

	@Resource
	private ICommonWordsTypeDao commonWordsTypeDao;
	@Resource
	private ICommonWordsInfoService commonWordsInfoService;
	...
	public AddWordsTypeReturn addWordsType(
		...
		try {
			...
			commonWordsTypeDao.insertSelective(wordsType);
	...
	public DelWordsTypeReturn delWordsType(
		...
		try {
			commonWordsTypeDao.deleteByPrimaryKey(delWordsType.getCode());
			DelWordsTypeReturn delWordsTypeReturn  = new DelWordsTypeReturn();
			
			//删除话术类型,需同步删除属于该话术类型的所有话术信息
			commonWordsInfoService.deleteWordsInfoByTypeCode(delWordsType.getCode());
	...
	public UpdateWordsTypeReturn updateWordsType(
		...
		try {
			...
			AssertUtils.notUpdateMoreThanOne(commonWordsTypeDao.updateByPrimaryKeySelective(wordsType));
			...
			if(StringUtils.isNotEmpty(updateWordsType.getTypeName()) && !updateWordsType.getTypeName().equals(findWordsTypeReturn.getTypeName())){
				...
				commonWordsInfoService.updateTypeName(updateWordsInfo);
			}
	...
	public FindWordsTypeReturn findWordsType(
		...
		try {
			WordsType wordsType = commonWordsTypeDao.selectByPrimaryKey(findWordsType.getCode());
			if(wordsType == null){
	...
	public Page<FindWordsTypePageReturn> findWordsTypePage(
		...
		int count = 0;
		try {
			returnList = commonWordsTypeDao.findWordsTypePage(findWordsTypePage);
			count = commonWordsTypeDao.findWordsTypePageCount(findWordsTypePage);
		}  catch (Exception e) {
	...
	public List<FindWordsAppReturn> findWords(FindWordsInfoApp findWordsInfoApp)
		...
		try {
			returnList = commonWordsTypeDao.findWords(findWordsInfoApp);
		}  catch (Exception e) {
	...
	public List<FindWordsTypeSelectReturn> findWordsTypes(
		...
		try {
			returnList = commonWordsTypeDao.findWordsTypes(findWordsInfoWeb);
		}  catch (Exception e) {
			logger.error("话术管理信息不存在错误",e);
			throw new TsfaServiceException(ErrorCode.WORDS_TYPE_FIND_PAGE_ERROR,"话术管理信息不存在错误.！",e);
		}
	...
	@Override
	public int hasTypeName(String merchantNo,String typeName) {
		//AssertUtils.notNullAndEmpty(merchantNo,"商户编号不能为空");
		AssertUtils.notNullAndEmpty(typeName,"类型名称不能为空");
		return commonWordsTypeDao.hasTypeName(merchantNo,typeName);
	}
	
	@Override
	public int hasSeq(String merchantNo,Integer seq) {
		//AssertUtils.notNullAndEmpty(merchantNo,"商户编号不能为空");
		AssertUtils.notNullAndEmpty(seq,"排序不能为空");
		return commonWordsTypeDao.hasSeq(merchantNo,seq);
	}
	...
	public int incrementTypeCountByPrimaryKey(String code, Integer increment) {
		...
		return commonWordsTypeDao.incrementTypeCountByPrimaryKey(code,increment);
	}
