...
//				guidMemberIntegralDto.setShopNo(member.getShopNo());
	...
	public DelGreetClientReturn delGreetClient(
		...
		try {
			...
			logger.debug("delGreetClient(DelGreetClient) - end - return value={}", delGreetClientReturn); 
	...
	public UpdateGreetClientReturn updateGreetClient(
		...
		logger.debug("updateGreetClient(UpdateGreetClient updateGreetClient={}) - start", updateGreetClient); 
		...
		try {
			...
			logger.debug("updateGreetClient(UpdateGreetClient) - end - return value={}", updateGreetClientReturn); 
	...
	public FindGreetClientReturn findGreetClient(
		...
		logger.debug("findGreetClient(FindGreetClient findGreetClient={}) - start", findGreetClient); 
		...
		try {
			...
			logger.debug("findGreetClient(FindGreetClient) - end - return value={}", findGreetClientReturn); 
	...
	public Page<FindGreetClientPageReturn> findGreetClientPage(
			...
			throws TsfaServiceException {
		logger.debug("findGreetClientPage(FindGreetClientPage findGreetClientPage={}) - start", findGreetClientPage); 
		...
		logger.debug("findGreetClientPage(FindGreetClientPage) - end - return value={}", returnPage); 
		return  returnPage;
	}
	...
	public FindGreetClientReturnDto findGreetClientForDay(FindGreetClientForDayDto findGreetClientForDayDto) throws TsfaServiceException {
		logger.debug("findGreetClientForDayDto(findGreetClientForDayDto findGreetClientForDayDto={}) - start", findGreetClientForDayDto); 
	...
	@Override
	public AddGreetClientReturn addGreetClients(AddGreetClient addGreetClient)
			throws TsfaServiceException {
		logger.debug("addGreetClient(AddGreetClient addGreetClient={}) - start", addGreetClient); 
		AssertUtils.notNull(addGreetClient);
		AssertUtils.notNullAndEmpty(addGreetClient.getMemberNoGm(),"导购编号不能为空");
		if(SendType.ALL != addGreetClient.getSendType()) {
			AssertUtils.notNull(addGreetClient.getMemberNo(),"会员编号不能为空");
		}
		try {
			if(SendType.MULTI == addGreetClient.getSendType()) {	// 发送给多人
				String[] mbrNosStrings = addGreetClient.getMemberNo().split(",");
				for(String memberNo : mbrNosStrings) {
					this.addGreetClient(addGreetClient, memberNo);
				}
			} else { // 发送单人或所有
				this.addGreetClient(addGreetClient, addGreetClient.getMemberNo());
			}
			logger.debug("addGreetClient(AddGreetClient) - end - return value={}", addGreetClient); 
			AddGreetClientReturn addGreetClientReturn = new AddGreetClientReturn();
			return addGreetClientReturn;
		}catch (TsfaServiceException e) {
			logger.error(e.getMessage(),e);
	...
	private void addGreetClient(AddGreetClient addGreetClient, String memberNo) {
		GreetClient greetClient = new GreetClient();
		//add数据录入
		greetClient.setCode(GUID.generateCode());
		greetClient.setMerchantNo(addGreetClient.getMerchantNo());
		greetClient.setMemberNo(memberNo);
		greetClient.setMemberNoGm(addGreetClient.getMemberNoGm());
		greetClient.setMemberNameGm(addGreetClient.getMemberNameGm());
		greetClient.setTitle(addGreetClient.getTitle());
		greetClient.setContent(addGreetClient.getContent());
