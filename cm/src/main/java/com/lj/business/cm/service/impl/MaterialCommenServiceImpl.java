...
public class MaterialCommenServiceImpl implements IMaterialCommenService { 
	...
	public AddMaterialCommenReturn addMaterialCommen(AddMaterialCommen addMaterialCommen) throws TsfaServiceException {
		...
		try {
			...
			// add数据录入
			materialCommen.setCode(addMaterialCommen.getCode());
			materialCommen.setStartTime(addMaterialCommen.getStartTime());
			materialCommen.setEndTime(addMaterialCommen.getEndTime());
...
//			materialCommen.setShopName(addMaterialCommen.getShopName());
	...
	public DelMaterialCommenReturn delMaterialCommen(
		...
		try {
			...
			logger.debug("delMaterialCommen(DelMaterialCommen) - end - return value={}", delMaterialCommenReturn); 
	...
	public UpdateMaterialCommenReturn updateMaterialCommen(
			...
			throws TsfaServiceException {
		logger.debug("updateMaterialCommen(UpdateMaterialCommen updateMaterialCommen={}) - start", updateMaterialCommen); 
		...
		try {
			...
			materialCommen.setStartTime(updateMaterialCommen.getStartTime());
			materialCommen.setEndTime(updateMaterialCommen.getEndTime());
...
//			materialCommen.setShopName(updateMaterialCommen.getShopName());
			...
			logger.debug("updateMaterialCommen(UpdateMaterialCommen) - end - return value={}", updateMaterialCommenReturn); 
	...
	public FindMaterialCommenReturn findMaterialCommen(
		...
		logger.debug("findMaterialCommen(FindMaterialCommen findMaterialCommen={}) - start", findMaterialCommen); 
		...
		try {
			...
			findMaterialCommenReturn.setStartTime(materialCommen.getStartTime());
			findMaterialCommenReturn.setEndTime(materialCommen.getEndTime());
...
//			findMaterialCommenReturn.setShopName(materialCommen.getShopName());
			...
			logger.debug("findMaterialCommen(FindMaterialCommen) - end - return value={}", findMaterialCommenReturn); 
	...
	public Page<FindMaterialCommenPageReturn> findMaterialCommenPage(
		...
		logger.debug("findMaterialCommenPage(FindMaterialCommenPage) - end - return value={}", returnPage); 
		return  returnPage;
	}
	...
	@Override
	public List<MaterialCommenGroup> findMaterialCommenByMerchantNo(
			FindMaterialCommenPage findMaterialCommenPage) {
		logger.debug("findMaterialCommenByMerchantNo(FindMaterialCommenPage findMaterialCommenPage={}) - start", findMaterialCommenPage);
		try {
			List<MaterialCommenGroup> materialCommenGroupList = new ArrayList<>();
			
			Map<String, List<FindMaterialCommenPageReturn>> map = new HashMap<>();//key:typeCode
			List<FindMaterialCommenPageReturn> materialCommenList = materialCommenDao.findMaterialCommenList(findMaterialCommenPage);

			String commonUrl = localCacheSystemParams.getSystemParam("api", "common", "commonUrl");
			commonUrl = commonUrl.endsWith("/") ? commonUrl.substring(0, commonUrl.length() -1) : commonUrl;
			for (FindMaterialCommenPageReturn findMaterialCommenPageReturn : materialCommenList) {
				if (map.containsKey(findMaterialCommenPageReturn.getMaterialTypeCode())) {
					map.get(findMaterialCommenPageReturn.getMaterialTypeCode()).add(findMaterialCommenPageReturn);
				} else {
					MaterialCommenGroup materialCommenGroup = new MaterialCommenGroup();
					List<FindMaterialCommenPageReturn> dataList = new ArrayList<>();
					materialCommenGroup.setMaterialTypeCode(findMaterialCommenPageReturn.getMaterialTypeCode());
					materialCommenGroup.setMaterialTypeName(findMaterialCommenPageReturn.getMaterialTypeName());
					dataList.add(findMaterialCommenPageReturn);
					materialCommenGroup.setDatas(dataList);
					
					map.put(findMaterialCommenPageReturn.getMaterialTypeCode(), dataList);
					
					materialCommenGroupList.add(materialCommenGroup);
				}
				//素材默认icon
				if(StringUtils.isBlank(findMaterialCommenPageReturn.getImgAddr())){
					//缺省 取商户LOGO
					FindMerchantDto findMerchantDto=new FindMerchantDto();
					findMerchantDto.setMerchantNo(findMaterialCommenPage.getMerchantNo());
					FindMerchantReturnDto selectMerchant = merchantService.selectMerchant(findMerchantDto);
					findMaterialCommenPageReturn.setImgAddr(selectMerchant.getLogoAddr());
				}else{
					//取素材表图片地址第一个
					String[] split = findMaterialCommenPageReturn.getImgAddr().split(",");
					findMaterialCommenPageReturn.setImgAddr(split[0]);
				}
				/*外部连接为空，访问自己的H5*/
				if(com.lj.base.core.util.StringUtils.isNullOrEmpty(findMaterialCommenPageReturn.getLinkUrl())){
					findMaterialCommenPageReturn.setUrlH5(commonUrl + "/oms-web/a/business/materialcommen/viewH5?code=" + findMaterialCommenPageReturn.getCode());
				}else{
					findMaterialCommenPageReturn.setUrlH5(findMaterialCommenPageReturn.getLinkUrl());
				}
			}
			
			return materialCommenGroupList;
		}catch (TsfaServiceException e) {
			throw e;
		} catch (Exception e) {
			logger.error("查找公用素材中心表信息信息错误！",e);
