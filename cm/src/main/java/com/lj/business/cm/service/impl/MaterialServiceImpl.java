...
public class MaterialServiceImpl implements IMaterialService { 
	...
	public AddMaterialReturn addMaterial(AddMaterial addMaterial) throws TsfaServiceException {
		...
		try {
			...
			material.setCode(addMaterial.getCode());
			material.setStartTime(addMaterial.getStartTime());
			material.setEndTime(addMaterial.getEndTime());
...
//				guidMemberIntegralDto.setShopNo(member.getShopNo());
	...
	public DelMaterialReturn delMaterial(
		...
		try {
			...
			logger.debug("delMaterial(DelMaterial) - end - return value={}", delMaterialReturn); 
	...
	public  UpdateMaterialReturn updateMaterial(
			UpdateMaterial updateMaterial)
					throws TsfaServiceException {
		logger.debug("updateMaterial(UpdateMaterial updateMaterial={}) - start", updateMaterial); 
		...
		try {
			...
			material.setStartTime(updateMaterial.getStartTime());
			material.setEndTime(updateMaterial.getEndTime());
			...
			logger.debug("updateMaterial(UpdateMaterial) - end - return value={}", updateMaterialReturn); 
	...
	public FindMaterialReturn findMaterial(
		...
		logger.debug("findMaterial(FindMaterial findMaterial={}) - start", findMaterial); 
		...
		try {
			...
			findMaterialReturn.setStartTime(material.getStartTime());
			findMaterialReturn.setEndTime(material.getEndTime());
			...
			logger.debug("findMaterial(FindMaterial) - end - return value={}", findMaterialReturn); 
	...
	public Page<FindMaterialPageReturn> findMaterialPage(
			FindMaterialPage findMaterialPage)
					throws TsfaServiceException {
		logger.debug("findMaterialPage(FindMaterialPage findMaterialPage={}) - start", findMaterialPage); 
		...
		logger.debug("findMaterialPage(FindMaterialPage) - end - return value={}", returnPage); 
		return  returnPage;
	} 
	...
	public Page<FindMaterialAppPageReturn> findMaterialAppPage(FindMaterialAppPage findMaterialAppPage)
			throws TsfaServiceException {
		logger.debug("findMaterialAppPage(FindMaterialAppPage findMaterialAppPage={}) - start", findMaterialAppPage); 
			...
			for (int i = 0; i < returnList.size(); i++) {
				...
				if ("COMMEN".equals(findMaterialAppPageReturn.getCommenMaterial())) {
					...
					if(StringUtils.isNullOrEmpty(findMaterialAppPageReturn.getLinkUrl())){
						findMaterialAppPageReturn.setUrlH5(commonUrl + "/oms-web/a/business/materialcommen/viewH5?code=" + findMaterialAppPageReturn.getCode());
					}else{
					...
					/*外部连接为空，访问自己的H5*/
					if(StringUtils.isNullOrEmpty(findMaterialAppPageReturn.getLinkUrl())){
						findMaterialAppPageReturn.setUrlH5(commonUrl + "/oms-web/a/business/material/viewH5?code=" + findMaterialAppPageReturn.getCode());
					}else{
						findMaterialAppPageReturn.setUrlH5(findMaterialAppPageReturn.getLinkUrl());
					}
				}
		...
		logger.debug("findMaterialAppPage(FindMaterialAppPage) - end - return value={}", returnPage); 
		return returnPage;
	}
	...
	@Override
	public List<MaterialGroup> findMaterialByGm(FindMaterial findMaterial) {
		try {
			List<MaterialGroup> materialGroupList = new ArrayList<>();
			Map<String, List<FindMaterialPageReturn>> map = new HashMap<>();
			String commonUrl = localCacheSystemParams.getSystemParam("api", "common", "commonUrl");
			commonUrl = commonUrl.endsWith("/") ? commonUrl.substring(0, commonUrl.length() -1) : commonUrl;
			
			List<FindMaterialPageReturn> list = materialDao.findMaterialSelect(findMaterial);
			for (FindMaterialPageReturn findMaterialPageReturn : list) {
				if (map.containsKey(findMaterialPageReturn.getMaterialTypeCode())) {
					map.get(findMaterialPageReturn.getMaterialTypeCode()).add(findMaterialPageReturn);
				} else {
					MaterialGroup materialGroup = new MaterialGroup();
					materialGroup.setMaterialTypeCode(findMaterialPageReturn.getMaterialTypeCode());
					materialGroup.setMaterialTypeName(findMaterialPageReturn.getMaterialTypeName());
					List<FindMaterialPageReturn> dataList = new ArrayList<>();
					dataList.add(findMaterialPageReturn);
					materialGroup.setDatas(dataList);
					map.put(findMaterialPageReturn.getMaterialTypeCode(), dataList);
					materialGroupList.add(materialGroup);
				}
				//素材默认icon
				if(StringUtils.isEmpty(findMaterialPageReturn.getImgAddr())){
					//缺省 取商户LOGO
					FindMerchantDto findMerchantDto=new FindMerchantDto();
					findMerchantDto.setMerchantNo(findMaterialPageReturn.getMerchantNo());
					FindMerchantReturnDto selectMerchant = merchantService.selectMerchant(findMerchantDto);
					if(selectMerchant!=null){
						findMaterialPageReturn.setImgAddr(selectMerchant.getLogoAddr());
					}
				}else{
					//取素材表图片地址第一个
					String[] split = findMaterialPageReturn.getImgAddr().split(",");
					findMaterialPageReturn.setImgAddr(split[0]);
				}
				/*外部连接为空，访问自己的H5*/
				if(StringUtils.isNullOrEmpty(findMaterialPageReturn.getLinkUrl())){
					findMaterialPageReturn.setUrlH5(commonUrl + "/oms-web/a/business/material/viewH5?code=" + findMaterialPageReturn.getCode());
				}else{
					findMaterialPageReturn.setUrlH5(findMaterialPageReturn.getLinkUrl());
				}
			}
