...
import com.lj.business.cm.dao.ICommonMaterialTypeDao;
 ...
 */
@Service
public class CommonMaterialTypeServiceImpl implements ICommonMaterialTypeService { 

	
	/** Logger for this class. */
	private static final Logger logger = LoggerFactory.getLogger(CommonMaterialTypeServiceImpl.class);
	

	/** The material type dao. */
	@Resource
	private ICommonMaterialTypeDao commonMaterialTypeDao;
	...
	public AddMaterialTypeReturn addMaterialType(AddMaterialType addMaterialType) throws TsfaServiceException {
		...
		try {
			...
			commonMaterialTypeDao.insertSelective(materialType);
	...
	public DelMaterialTypeReturn delMaterialType(
		...
		try {
			commonMaterialTypeDao.deleteByPrimaryKey(delMaterialType.getId());
	...
	public UpdateMaterialTypeReturn updateMaterialType(UpdateMaterialType updateMaterialType,FindMaterialTypeReturn findMaterialTypeReturn)
		...
		try {
			...
			AssertUtils.notUpdateMoreThanOne(commonMaterialTypeDao.updateByPrimaryKeySelective(materialType));
			...
			if(StringUtils.isNotEmpty(updateMaterialType.getTypeName())&&!updateMaterialType.getTypeName().equals(findMaterialTypeReturn.getTypeName())){
				...
				materialCommenDao.updateTypeName(updateMaterial);
			}
	...
	public FindMaterialTypeReturn findMaterialType(FindMaterialType findMaterialType) throws TsfaServiceException {
		...
		try {
			MaterialType materialType = commonMaterialTypeDao.selectByPrimaryKey(findMaterialType.getCode());
	...
	public Page<FindMaterialTypePageReturn> findMaterialTypePage(FindMaterialTypePage findMaterialTypePage)
		...
		int count = 0;
		try {
			returnList = commonMaterialTypeDao.findMaterialTypePage(findMaterialTypePage);
			count = commonMaterialTypeDao.findMaterialTypePageCount(findMaterialTypePage);
		} catch (Exception e) {
	...
	public List<FindMaterialTypesAppReturn> findMaterialTypesApp(FindMaterialTypesApp findMaterialTypesApp) {
		...
		try {
			List<FindMaterialTypesAppReturn> list = commonMaterialTypeDao.findMaterialTypesApp(findMaterialTypesApp);
	...
	public int incrementTypeCountByPrimaryKey(String code, Integer increment) {
		...
		return commonMaterialTypeDao.incrementTypeCountByPrimaryKey(code, increment);
	}
	...
	public int decrementTypeCountByPrimaryKey(String code, Integer decrement) {
		...
		return commonMaterialTypeDao.decrementTypeCountByPrimaryKey(code, decrement);
	}
	...
	public Page<FindMaterialTypePageReturn> findMaterialTypeForMemberPage(FindMaterialTypePage findMaterialTypePage)
		...
		int count = 0;
		try {
			returnList = commonMaterialTypeDao.findMaterialTypeForMemberPage(findMaterialTypePage);
...
//				countMaterialCommenDto.setShopNo(findMaterialTypePage.getShopNo());
			...
			count = commonMaterialTypeDao.findMaterialTypeForMemberPageCount(findMaterialTypePage) + 1;
		} catch (Exception e) {
			logger.error("素材类型表信息分页查询错误", e);
			throw new TsfaServiceException(ErrorCode.MATERIAL_TYPE_FIND_PAGE_ERROR, "素材类型表信息分页查询错误.！", e);
		}
	...
	public List<FindMaterialTypePageReturn> findMateriaTypeList(
		...
		try {
			list=commonMaterialTypeDao.findMateriaTypeList(findMaterialTypePage);
		} catch (Exception e) {
			logger.error("素材类型表信息分页查询错误", e);
			throw new TsfaServiceException(ErrorCode.MATERIAL_TYPE_FIND_PAGE_ERROR, "素材类型表信息分页查询错误.！", e);
		}
