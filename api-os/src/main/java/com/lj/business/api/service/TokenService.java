...
import com.lj.business.api.domain.member.MemberLoginCache;
...
public class TokenService {
	...
	public Token generateToken(String memberNoGm, String appKey,int tokenTimeout) {
		...
		distributeCache.set(getRefreshCacheKey(refreshToken), memberNoGm, TOKEN_TIMEOUT_SECONDS);
		...
		distributeCache.set(memberKey, refreshToken + accessToken, TOKEN_TIMEOUT_SECONDS);
	...
	public void removeToken(String accessToken) {
		...
		// 删除会员令牌和刷新令牌
		String memberKey = getMemberCacheKey(memberCache.getMemberNoGm(), memberCache.getAppKey());
		String memberToken = distributeCache.get(memberKey);
		if(StringUtils.isNotEmpty(memberToken)) {
			distributeCache.del(getRefreshCacheKey(memberToken.substring(0, REFRESH_LENGTH)));	// 删除刷新令牌
