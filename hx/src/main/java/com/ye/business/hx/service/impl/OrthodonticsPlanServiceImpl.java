...
public class OrthodonticsPlanServiceImpl implements IOrthodonticsPlanService {
	...
	public void addOrthodonticsPlan(OrthodonticsPlanDto orthodonticsPlanDto) throws TsfaServiceException {
		...
		try {
			...
			if(orthodonticsPlanDto.getType()==4 && orthodonticsPlanDto.getSort() != null) {
				orthodonticsPlan.setSort(orthodonticsPlanDto.getSort());
			}
			...
			orthodonticsPlan.setContent(orthodonticsPlanDto.getName());
			orthodonticsPlan.setPatientNo(orthodonticsPlanDto.getPatientNo());
	 ...
	 *
	 */
	public List<OrthodonticsPlanDto> findOrthodonticsPlans(FindOrthodonticsPlanPage findOrthodonticsPlanPage)
			throws TsfaServiceException {
	...
	public void updateOrthodonticsPlan(OrthodonticsPlanDto orthodonticsPlanDto) throws TsfaServiceException {
		...
		try {
			...
			orthodonticsPlan.setRemark4(orthodonticsPlanDto.getRemark4());
			orthodonticsPlan.setContent(orthodonticsPlanDto.getName());
			orthodonticsPlan.setUpdateDate(new Date());
			if (orthodonticsPlanDto.isMove()) {
				orthodonticsPlanDao.updateSort(orthodonticsPlanDto);
				orthodonticsPlan.setSort(orthodonticsPlanDto.getNextSort());
			}
	...
	@Override
	public OrthodonticsPlanDto findOrthodonticsPlan(OrthodonticsPlanDto orthodonticsPlanDto)
			throws TsfaServiceException {
		...
		try {
			OrthodonticsPlan orthodonticsPlan = new OrthodonticsPlan();
			if (StringUtils.isNotEmpty(orthodonticsPlanDto.getCode())) {
				orthodonticsPlan = orthodonticsPlanDao.selectByPrimaryKey(orthodonticsPlanDto.getCode());
			} else {
				orthodonticsPlan = orthodonticsPlanDao.selectByPatientNo(orthodonticsPlanDto.getPatientNo());
			}
			if (orthodonticsPlan == null) {
				return null;
				// throw new
				// TsfaServiceException(ErrorCode.ORTHODONTICS_PLAN_NOT_EXIST_ERROR,"正畸计划配置信息不存在");
			}
			...
			findOrthodonticsPlanReturn.setName(orthodonticsPlan.getContent());
			findOrthodonticsPlanReturn.setSort(orthodonticsPlan.getSort());
			findOrthodonticsPlanReturn.setPatientNo(orthodonticsPlan.getPatientNo());

			logger.debug("findOrthodonticsPlan(OrthodonticsPlanDto) - end - return value={}",
					findOrthodonticsPlanReturn);
	...
	@Override
	public Page<OrthodonticsPlanDto> findOrthodonticsPlanPage(FindOrthodonticsPlanPage findOrthodonticsPlanPage)
			throws TsfaServiceException {
		logger.debug("findOrthodonticsPlanPage(FindOrthodonticsPlanPage findOrthodonticsPlanPage={}) - start",
				findOrthodonticsPlanPage);
		...
		Page<OrthodonticsPlanDto> returnPage = new Page<OrthodonticsPlanDto>(returnList, count,
				findOrthodonticsPlanPage);
	...
	public Integer getMaxSort() {
		Integer sort = orthodonticsPlanDao.selectMaxSort();
		return sort;
	}
