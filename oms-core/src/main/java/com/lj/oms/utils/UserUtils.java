...
public class UserUtils {
	...
	public static final String USER_GAIN_MERCHANT_NO="userGainMerchantNo";
	public static final String USER_SHOP_NO_S = "userShopNos";
	//登录缓存
	public static final  String  LOGIN_TOKEN_TYPE="user_login_token";
	...
	public static User get(String id){
		...
		if (user ==  null){
			...
			CacheUtils.put(USER_SHOP_NO_S, user);
	...
	public static void clearCache(User user){
		...
		CacheUtils.remove(USER_GAIN_MERCHANT_NO);
	 ...
	 */
	public static List<Menu> getMenuListByUserid(String userid) {
		@SuppressWarnings("unchecked")
		List<Menu> menuList = (List<Menu>) CacheUtils.get(USER_CACHE, CACHE_MENU_LIST + userid);
		if (menuList == null) {
			...
			CacheUtils.put(USER_CACHE, CACHE_MENU_LIST + userid, menuList);
		}
		return menuList;
	}
	 ...
	 */
	@SuppressWarnings("unchecked")
	public static List<Office> getOfficeListByUserId(String userId) {
		List<Office> officeList = (List<Office>) getCache(CACHE_OFFICE_LIST);
		if (officeList == null) {
			User user = get(userId);
			if (user.isAdmin()) {
...
//	}
	 ...
	 * @return
	 *
	 * @author 段志鹏 CreateDate: 2018年3月26日
	 *
	 */
	public static String getMerchantNo(){
		User user = getUser();
		if (null != user){
			//如果用户角色id为2,则为总管账号,可以查看所有商户信息
