 ...
 */
@Controller
@RequestMapping(value = "${adminPath}/cm/commonWordsInfo")
public class CommonWordsInfoController  extends BaseController{
	
	
	private static final Logger logger = LoggerFactory.getLogger(CommonWordsInfoController.class);

	@Resource
	private ICommonWordsInfoService commonWordsInfoService;			//话术服务
	@Resource
	...
	public String list(FindWordsInfoPage findWordsInfoPage,Integer pageNo,Integer pageSize, Model model) {
		try {
			...
			List<FindWordsTypeSelectReturn> typeList=commonWordsTypeService.findWordsTypes(findWordsInfoWeb);
			model.addAttribute("typeList",typeList);
			
		} catch (Exception e) {
			logger.error("获取话术信息错误！",e);
		}
		return "modules/cm/commonWordsInfoList";
	...
	public String form(FindWordsInfo findWordsInfo, Model model) {
		try {
			if(findWordsInfo!=null && findWordsInfo.getCode()!=null){
				if(StringUtils.isNotBlank(findWordsInfo.getCode())){
					FindWordsInfoReturn findWordsInfoReturn= commonWordsInfoService.findWordsInfo(findWordsInfo);
		...
		return "modules/cm/commonWordsInfoForm";
	}
	...
	public String save(AddWordsInfo addWordsInfo, Model model, RedirectAttributes redirectAttributes) {
		...
		try {
			...
			Integer count=commonWordsInfoService.findDefaultCount(addWordsInfo.getMerchantNo());
			...
			commonWordsInfoService.addWordsInfo(addWordsInfo);
			addMessage(redirectAttributes, "保存话术成功");
		} catch (Exception e) {
			logger.error("保存话术信息错误！",e);
		}
		return "redirect:" + adminPath + "/cm/commonWordsInfo/";
	}
	...
	public String edit(UpdateWordsInfo updateWordsInfo, Model model, RedirectAttributes redirectAttributes) {
		...
		try {
			...
			if("0".equals(findWordsInfoReturn.getDefaultFlag().toString())&&"1".equals(updateWordsInfo.getDefaultFlag().toString())){
				Integer count=commonWordsInfoService.findDefaultCount(UserUtils.getUser().getCompany().getId());
			...
			commonWordsInfoService.updateWordsInfo(updateWordsInfo);
	...
	public String delete(DelWordsInfo delWordsInfo, Model model, RedirectAttributes redirectAttributes) {
		FindWordsInfoPage findWordsInfoPage=new FindWordsInfoPage();
		try {
			commonWordsInfoService.delWordsInfo(delWordsInfo);
