...
function saveFriendsVideo(form,code){
	var contentLength = $('#content').val();
	var commentContentVal = $('#commentContent').val();
	var title=$("#title").val();
	if(title.length>100){
	...
	if(contentLength.length>1800){
		...
		$("#content").val(cc);
		if(code == ''){ // 新增
			$(form).attr('action',$('#ctx').val() + '/im/addFriendsVideoMaterial');
		}else{ // 修改
			$(form).attr('action',$('#ctx').val() + '/im/updataVideoMaterial');
		}
		$(form).submit();
	}
