...
clock = function (memberNo,noWxGm,num)
{
	console.log("定时器执行："+memberNo+","+noWxGm);
	$.ajax({
        type:"POST",
	...
	$.ajax({
        ...
        data:{noWxGm:noWxGm,memberNo:memberNo,limit:limit,chatTimeEnd:chatTimeEnd},
        dataType:'JSON',
        success:function(result){
    		if(result.total>0){
    				...
    				if(list[i-1].senderFlag==2){	//客户发送
    					...
    					if(list[i-1].type==1){			//文本
    						...
    						$.ajax({
    						    ...
    						    async: false,
    						    success:function(re){
    						    	html += '<div class="information"><div class="audio" onclick="play(this)"><audio src="'+re+'"  class="yuying"></audio></div></div><div class="seCo">'+(JSON.parse( list[i-1].content).duration/1000).toFixed(0)+'"</div></div>';
    						    }
    						});
    					...
    					if(list[i-1].type==1){			//文本
    						...
    						html += '<div class="shareInfo removeInfo" data-code='+list[i-1].code+' data-Time='+list[i-1].chatTime+' onclick=\'javascript:showContent2("'+encodeURI(list[i-1].shareUrl)+'","yulan") \'><div class="sendDec"><img alt=""  src="'+gudHeadAddress+'">';
    						html += '<div class="sendDecText"><p>导购姓名：'+list[i-1].shareTitle +'</p>';
    						html += '<p>'+list[i-1].shareDes.split("\n")[0]+'</p>';
    						if(list[i-1].shareDes.split("\n")[1]!=undefined){
    							html +='<p>'+list[i-1].shareDes.split("\n")[1]+'</p>';
    						...
    						var lng = JSON.parse(list[i-1].content).lng;
    						html += '<div class="mine personCard sendAll">';
    						html +='<div class="mapToy"><div class="mapCon removeInfo" data-code='+list[i-1].code+' data-Time='+list[i-1].chatTime+'  onclick="clickMap(\''+lat+'\',\''+lng+'\',\''+label+'\')"><h2 class="mapToy-title">'+ poiname +'</h2><span class="mapToyMs">'+ label +'</span><span class="mapToyMs">'+ "" +'</span><div id="'+ list[i-1].code +'" class="containerMap">' +'</div></div><img alt="" class="headImg" src="'+gudHeadAddress+'"></div></div>';
    						mapList.push(list[i-1]);
    					}else if(list[i-1].type==436207665){
    						var content = JSON.parse(list[i-1].content);
        ...
        success:function(result){
        	if(result!=null){
            	...
            	if(result.headAddress!=""){
            		...
            		$(".other .headImg").attr("src",result.headAddress);
            	}
