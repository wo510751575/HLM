...
function page(n,s){
	$("#pageNo").val(n);
	$("#pageSize").val(s);
	$("#searchForm").attr("action","${ctx}/weixin/chatInfo");
	$("#searchForm").submit();
	return false;
}
</script>
...
<body>
	...
	<ul class="nav nav-tabs">
		<li><a href="${ctx}/weixin/imChatInfo/?talker=${parmMap['talker']}&memberNoGm=${parmMap['memberNoGm']}">IM聊天记录</a></li>
<%-- 		<li class="active"><a href="${ctx}/weixin/chatInfo/?talker=${parmMap['talker']}&memberNoGm=${parmMap['memberNoGm']}">微信聊天记录</a></li> --%>
	</ul>
	<form id="searchForm" action="${ctx}/weixin/chatInfo/" method="post"
			...
			<c:forEach items="${page.list}" var="item">
				<tr>
					...
					<td>${item.num}</td>
			          ...
			          <c:when test="${item.type =='1'} || ${item.type =='10000'}">
		...
		$('.view_btn').click(function() {
			...
			$("#startTimeDetail").val(startTime);
			 $("#chatDetail ul li").remove();
			  $.ajax({
                 type:"POST",
                 url:"${ctx}/weixin/chatInfo/view",
                 data:{talker:talker,startTime:startTime,pageSize:11,memberNo:memberNo},
                 dataType:'JSON',
                	 ...
                	 if(result!=undefined&& result.length>0){
                		 ...
                		 for(var i=result.length-1;i>=0;i--){
                			 if(isEmptyObject(result[i].gmPhoto)){
             					result[i].gmPhoto="http://112.74.55.63/headImg/gm_head.jpg";
             				}
                			 if(result[i].issend==1){  //issend==1表示是自己发的信息
                				 var info1='';
                			 //消息类型 1文本 3图片 34语音 43视频 42名片 47图片表情 48定位 49分享 50视频聊天 10000系统信息（添加好友）318767153系统安全提示
                			     if(result[i].type=='49'){
                			    	 info1='<span class="send_info_right right">'+'<a href="'+result[i].shareUrl+'" target="_blank" title="'+result[i].shareTitle+'">[分享]</a>'+'</span>';
                			     }else if(result[i].type=='1'||result[i].type=='50'||result[i].type=='318767153'){
                			 	...
                			 	if(result[i].gmPhoto.indexOf("http")==-1){
                    				 ...
                    				 '" /></div><div class="mem_info right">'+info1+'</div></li>'
                			 		 ...
                			 		 html=html+'<li class="right"><div class="mem_head right"><img src="'+result[i].gmPhoto+ 
                    				 '" /></div><div class="mem_info right">'+info1+'</div></li>'
                			 	}
                				
                			 }else if(result[i].issend==0){ //issend==0表示是对方发的信息
                				 var info=""
                				     if(result[i].type=='49'){
                				 ...
                				 if(result[i].pmPhoto.indexOf("http")==-1){
                    				 html=html+'<li class="left"><div class="mem_head left"> <img src="'+uploadUrl+result[i].pmPhoto+ 
			  ...
			  $('#chatDetail').on('scroll',function(){
				  ...
				  if(sct==0 && lastPage=="false"){
					  ...
					  $.ajax({
			                 type:"POST",
			                 url:"${ctx}/weixin/chatInfo/view",
			                 data:{talker:$("#talkDetail").val(),startTime:$("#startTimeDetail").val(),pageNo:next,pageSize:11},
			                 dataType:'JSON',
			                 success:function(result){
			                	 ...
			                	 if(result!=undefined && result.length>0){
			                		 for(var i=result.length-1;i>=0;i--){
			                			 ...
			                			 if(result[i].issend==1){  //issend==1表示是自己发的信息
			                				 ...
			                				 if(result[i].type=='49'){
			                					info1='<span class="send_info_right right">'+'<a href="'+result[i].shareUrl+'" target="_blank" title="'+result[i].shareTitle+'">[分享]</a>'+'</span>';
			                     			 }else if(result[i].type=='1'||result[i].type=='50'||result[i].type=='10000'||result[i].type=='318767153'){
			                					info1='<span class="send_info_right right">'+result[i].content+'</span>';
			                   				 }else if(result[i].type=='3' ||result[i].type=='47'){
			                   					var resourcesPath='';
			                   					 if(result[i].resourcesPath){
