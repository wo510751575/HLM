		...
		$(document).ready(function() {
			...
			if(repMsg){
				if(isContains(repMsg,"成功")&& (isContains(repMsg,"保存")||isContains(repMsg,"编辑"))){
					...
					window.parent.frames[0].location.href="${ctx}/cm/commonWordsType/";//刷新指定iframe
					top.$.jBox.close(true);
				}else{
					showTip(repMsg);
					$("#repMsg").val("");
			...
			$('.typeSave').click(function() {
				...
				// 正常打开	
				top.$.jBox.open("iframe:${ctx}/cm/commonWordsType/form?code="+code,"", 380, 180,{//宽高
					id:7777,
					top:'25%',
					title:null,
					//height:'auto',
...
<div class="container">
	<ul class="nav nav-tabs">
		<li class="active"><a href="${ctx}/cm/commonWordsType/">公司话术类型列表</a></li>		
			<%-- <li><a href="${ctx}/cm/wordsType/form">话术类型添加</a></li> --%>	
			<li><a href="javascript:;" class="typeSave">新增公司话术类型</a></li>
	</ul>
	<form id="searchForm" action="${ctx}/cm/commonWordsType/" method="post" class="breadcrumb form-search">
				...
				<c:choose>
					<c:when test="${empty merchantNo}">
						<td>
							...
							&nbsp;&nbsp;<a href="${ctx}/cm/commonWordsType/delete?code=${item.code}&typeName=${item.typeName}" onclick="return confirmx('删除话术类型后，对应话术类型下的所有话术会被删除。仍确认要删除该话术类型吗？', this.href)">删除</a>&nbsp;&nbsp;
						</td>
					</c:when>
					<c:otherwise>
						<td>
