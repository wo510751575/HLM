	...
	<script type="text/javascript">
		$(document).ready(function() {
			...
			$("#btnExport").click(function(){
				...
				top.$.jBox.confirm("确认要导出经销商运营日报数据吗？","系统提示",function(v,h,f){
					if(v=="ok"){
						$("#searchForm").attr("action","${ctx}/st/companyOperationDayReport/export?export=1");
						$("#searchForm").submit();
					}
				},{buttonsFocus:1});
			});
		...
		function page(n,s){
			$("#pageNo").val(n);
			$("#pageSize").val(s);
			$("#searchForm").attr("action","${ctx}/st/companyOperationDayReport");
			$("#searchForm").submit();
        	return false;
        }
...
<div class="container">
	...
	<ul class="nav nav-tabs">
		<li class="active"><a href="${ctx}/st/companyOperationDayReport">经销商运营日报</a></li>
	</ul>
	<form id="searchForm" action="${ctx}/st/companyOperationDayReport" method="post" class="breadcrumb form-search">
				...
				<c:when test="${!isCompany}">
					<li><label>日期：</label>
						<input id="reportDate" name="reportDate" type="text" readonly="readonly" maxlength="20"  class="input-mini Wdate"
		 				value="<fmt:formatDate value="${findCompanyOperationDayReportPage.reportDate}" pattern="yyyy-MM-dd"/>" onclick="WdatePicker({dateFmt:'yyyy-MM-dd',isShowClear:false});"/> 
					</li>
				...
				<c:otherwise>
					<li><label>日期：</label>
						<input id="reportDateBegin" name="reportDateBegin" type="text" readonly="readonly" maxlength="20"  class="input-mini Wdate"
		 				value="<fmt:formatDate value="${findCompanyOperationDayReportPage.reportDateBegin}" pattern="yyyy-MM-dd"/>" onclick="WdatePicker({dateFmt:'yyyy-MM-dd',isShowClear:false});"/>&nbsp; 
						-&nbsp; <input id="reportDateEnd" name="reportDateEnd" type="text" readonly="readonly" maxlength="20"  class="input-mini Wdate" 
		 				value="<fmt:formatDate value="${findCompanyOperationDayReportPage.reportDateEnd}" pattern="yyyy-MM-dd"/>" onclick="WdatePicker({dateFmt:'yyyy-MM-dd',isShowClear:true});"/>&nbsp;&nbsp;
					</li>
				</c:otherwise>
			</c:choose>
			
			<li class="btns"><input id="btnSubmit" class="btn btn-primary" type="submit" value="查询" onclick="return page();"/></li>
			<shiro:hasPermission name="st:companyOperationDayReport:view">
