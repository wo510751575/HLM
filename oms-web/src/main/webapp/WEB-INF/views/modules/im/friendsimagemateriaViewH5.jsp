	...
	<div class="lj-right telescopicButton">
		...
		<form action="" id="formFriend" method="post">
	        ...
	        <div class="edit-material">
	            ...
	            <div class="tag">
	                <span class="n" style="line-height: 44px;">分享图片：</span>
	                <div class="">
	                    <div>
	                    	<span class="s-btn">选择图片
		                        <input type="file" id="fileId" class="file-btn" value="" name="imgFiles" multiple="multiple" onchange="ajaxFileUpload({fileId:'fileId',imgId:'#imgListId',max:9,isCompress:false})"/>
		                        <input type="hidden" value="" name="imgAddr" class="imgAddr" id="imgAddr"/>
	                    	</span>
	                        <span class="tip">没有任何图片时表示纯文字素材</span>
	                    </div>
	                    <p class="tip-num">最多选择9张</p>
	                    
	                    <ul class="img-list" id="imgListId">
	                    	<c:if test="${not empty data.imgAddr}">
		                    	<c:set value="${fn:split(data.imgAddr,',')}" var="img" />
	            ...
	            <div class="button-list" style="margin-top:50px;">
	                <input type="button" value="保存" class="b-btn" onclick="saveFriendsImage('#formFriend','${data.code }')"/>
	            </div>
	        </div>
...
<script src="${ctxStatic}/admin/js/im/friendsimagemateriaViewH5.js?v=11" type="text/javascript"></script>
</body>
</html>