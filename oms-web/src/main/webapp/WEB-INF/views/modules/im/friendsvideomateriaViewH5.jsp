...
<input type="hidden" value="video" id="MATERIALLISTTYPEID"/><!-- 素材类型列表 -->
	...
	<div class="lj-right telescopicButton">
		...
		<form action="" id="formFriend" method="post">
	        ...
	        <div class="edit-material">
	            ...
	            <div class="tag">
	                <span class="n" style="line-height: 44px;"><i>*</i>分享视频：</span>
	                <div class="">
	                    <div>
	                    	<span class="s-btn">选择视频
		                        <input type="file" id="fileId" accept="video/mp4" class="file-btn" value="" name="imgFiles" multiple="multiple" onchange="ajaxVideoUpload({fileId:'fileId',imgId:'#imgListId',max:1,isCompress:true})"/>
		                        <input type="hidden" value="" name="linkUrl" class="imgAddr" id="imgAddr"/>
	                    	</span>
	                    	 <span class="tip">视频时间最长10秒</span>
	                    </div>
	                    <div class="img-list">
	                    	<ul id="imgListId">
		                    	<c:if test="${not empty data.linkUrl}">
			                    	<c:set value="${fn:split(data.linkUrl,',')}" var="img" />
			                    	
				                    <c:forEach items="${img}" var="s">
				                        <li data-src="${s}">
				                       		<video style="width:300px" src="${fns:getUploadUrl()}im/${s}" controls="controls" ></video>
	            ...
	            <div class="button-list" style="margin-top:50px;">
	                <input type="button" value="保存" class="b-btn" onclick="saveFriendsVideo('#formFriend','${data.code }')"/>
	            </div>
	        </div>
		</form>
    </div>
...
<script src="${ctxStatic}/admin/js/im/friendsvideomateriaViewH5.js?v=11" type="text/javascript"></script>
</body>
</html>