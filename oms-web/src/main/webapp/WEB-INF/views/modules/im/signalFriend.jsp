...
<body>
	...
	<input type="hidden" value="${wxNickname }" id="wxNickname" />
	...
	<div class="imcontent sendF" id="chat">
	<input type="hidden" value="rp" id="LEFTNAVTYPEID"/><!-- 左边菜单类型 -->
	<%@ include file="/WEB-INF/views/modules/im/leftNav.jsp"%>
		<div class="shop_nav" style="width: 26.2%;">
			<div class="shop_search">
				<span class="back" onclick="javascript:history.go(-1)">
					返回
				</span>
			</div>
			<div class="shop_list" id="shop_list">
				<ul class="person_chat">
					...
					<li onclick="material('time',this)" class="text-center">时光长廊</li>
					</c:if>
				</ul>
				<!-- 个人资料 -->
				<div class="person_chat_info">
					...
					<input type="hidden" value="${personMember.noWx }" class="noWx" />
						<ul>
							<li><span>备注</span><input value="${personMember.memberName}" class="memAppName"  type="text"/></li>
							<li class="oneLine">
							<span>微信昵称</span><span class="nickNameWx">${personMember.nickNameWx }</span>
								<span class="peSex">(<c:if test="${personMember.sex=='MALE'}">男</c:if>
...
<%-- 							<li><span>跟进次数</span><span>${personMember.followNum}</span></li> --%>
		...
		<div class="con_right " id="friendScroll">
			<div class="con_right_title">最新朋友圈</div>
...
<script src="${ctxStatic}/admin/js/im/friendPerson.js?v=11" type="text/javascript"></script>
