...
<link rel="stylesheet" type="text/css" href="${ctxStatic}/bootstrap/3.0.2/bootstrap.min.css">
<link href="${ctxStatic}/bootstrap/2.3.1/awesome/font-awesome.min.css" type="text/css" rel="stylesheet" />
<link rel="stylesheet" type="text/css" href='${ctxStatic}/admin/css/theme_styles${layout==null?"_darkblue":layout}.css'>
...
#logo.logo-small{width: 64px;padding-left: 10px;}
			...
			<div class="container">
							<a href="${ctx}/sys/user/info" target="mainFrame" id="logo" class="navbar-brand logotitle"> 
						       ...
						       <c:if test="${empty user_photo}">   
									...
									<span style="top:35px;width: 124px;font-size:14px;line-height:1.5;top:35px" id="username">
										...
										<c:if test="${menu.parent.id eq '1' && menu.isShow eq '1'}">
										     <li class="menu_title menu pull-left ${firstMenu?'chose':''}">
												...
												<c:if test="${not empty menu.href}">
													...
													<a class="menu" href="${(fn:startsWith(menu.href, 'http://') or fn:startsWith(menu.href, 'https://') ) ? '' : ctx}${menu.href}" data-id="${menu.id}" target="${not empty menu.target?menu.target:'mainFrame'}"><span><i class="icon-${menu.icon}"></i>${menu.name}</span></a>
												</c:if>
											</li>
					...
					<div class="nav-no-collapse pull-right" id="header-nav">
						<ul class="nav navbar-nav pull-right collapse navbar-collapse navbar-ex1-collapse ">
						  ...
						  <li class="dropdown">
							...
							<!-- <li class="hidden-xxs_wx_OpenId" id="hidden-xxs_wx_OpenId">
								<a class="btn dropdown-toggle" data-toggle="hidden-xxs_wx_OpenId">
								<i class="icon-bookmark"></i>
								<span class="hidden-xs">初始换wxOpenId</span>
								</a>
							</li> -->
...
<!-- 							</li> -->
						...
						<%-- <c:if test="${!empty user_photo}">
						       ...
						       <c:if test="${empty user_photo}">
									...
									<span class="hidden-xs"><shiro:principal property="name"/></span> <b class="caret"></b>
							</a>
								<ul class="dropdown-menu">
									<li><a
			               ...
			               <!-- 超管才有 -->
			               <c:if test="${fns:getPrincipal().id eq '1'}">
				               	<li id="ids" class="dropdown profile-dropdown" >
				               		<a href="#" class="dropdown-toggle" data-toggle="dropdown"> 
										<span class="hidden-xs">筛选商户</span><b class="caret"></b>
									</a>
									<ul class="dropdown-menu">
										<c:forEach items="${list}" var="item">
				 			             	<li><a href="#">
												<i class="icon-group" data-id="${item.id}"></i>${item.name}</a>
											</li>
			 			               	</c:forEach>
...
<%-- 												</c:forEach> --%>
												<!-- 焕新子菜单不显示 -->
												<c:forEach items="${menuList}" var="menu2">
												<c:if test="${ !fn:contains(menu2.parentIds,'ebba8b7c400d46f78c220f702a17ba99') && menu2.parent.id eq (not empty menu.id ? menu.id:1)&&menu2.isShow eq '1'}">
				...
				<!-- right -->
				<div id="content-wrapper" style="-webkit-overflow-scrolling:touch; overflow: scroll; ">
					...
					<footer id="footer-bar" class="row" style="opacity: 1;">
						<p id="footer-copyright" class="col-xs-12">
							© 2018 <a href="#">${fns:getConfig('productName')}</a>. Powered by ${fns:getConfig('company')} ${fns:getConfig('version')}
						</p>
					</footer>
				</div>
			</div>
	...
	<script type="text/javascript">
		$(document).ready(function() {
			  $.ajax({
                  ...
                  success:function(result){
                      if(result!="YES"){
                    	  $("#ids").hide();
                    	  $("#hidden-xxs_wx_OpenId").hide();
                      }
                  }
              });
			  //根据商户编号初始化pmb表wxOpenId
			  $('#hidden-xxs_wx_OpenId').find('span').bind('click',function(){
				  var merchantName = $("#ids option:selected").text();
				  var merchantNo = $("#ids option:selected").val();
				  alert("初始化-"+merchantName+",的wxOpenId开始...");
				  $.post('${ctx}/index/initWxOpenIdByMerchantNo',{'merchantNo':merchantNo},function(data){
					  if(data){
						  alert("商户:"+merchantName+",初始化wxOpenId完成!");
					  }
				  });
			  });
			  
			  var usnm=$("#username").text();
			  if($("#username").text()!="" && $("#username").text().length > 5){
				 $("#username").css("font-size","14px");
				 $("#username").css("line-height","1.5");
				 $("#username").css("top","35px");
			  }
             
		    
		     $("#ids .dropdown-menu a").click(function(){
		    	 var merchantNo=$("#ids").val();
		    	  $.ajax({
		    		  type:"POST",
	                  url:"${ctx}/index/get",
	                  data:{merchantNo:merchantNo},
	                  success:function(result){ 
	                	$("#sidebar-nav li a")[0].click();
	                  }
		    	  });
		     });
		...
		function openNew(){//'location=no,menubar=no,status=no,toolbar=no'
			newWind= window.open('${ctx}/im/list','_blank') 
		}
	</script>
</body>
</html>