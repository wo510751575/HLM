...
<script type="text/javascript"
	src="${ctxStatic}/common/ajaxfileupload.js"></script>
<script type="text/javascript" src="${ctxStatic}/editor/kindeditor.js"></script>
<script type="text/javascript" src="${ctxStatic}/editor/init.js"></script>
<script src="${ctxStatic}/common/plupload.full.min.js"
	type="text/javascript"></script>
<link href="${ctxStatic}/admin/css/model.css" type="text/css"
	rel="stylesheet" />
<style type="text/css">
html, body {
	margin: 0;
...
.phone-title-seciton {
	...
	height: 44px;
	background:
		url(${fns:getUploadUrl()}/oms/image/materialcommen/phone_head_bg.png)
		no-repeat 0 0;
	background-size: 100%;
}
...
.modelOne_content {
	width: 100%;
	background: url(${fns:getUploadUrl()}/oms/image/materialcommen/model1_bg.png)
		no-repeat 0 0;
...
.modelOne_foot {
	width: 100%;
	background:
		url(${fns:getUploadUrl()}/oms/image/materialcommen/model1_foot.png)
		no-repeat 0 0;
	background-size: 100%;
	padding-bottom: 10px;
	height: 100px;
...
.modelTWO .md-t-hto .gj .t .n {
	font-size: 17px;
}

.modelTWO .md-t-phone .list li .n {
	font-size: 15px;
}

.modelTWO .md-t-phone .list li .d {
	font-size: 13px;
}
</style>
	...
	<c:choose>
		<c:when test="${temp!='' && temp!=null}">
 			${fns:unescapeHtml(data.content)}
   			...
   			<link href="${ctxStatic}/admin/css/models/ami-pro1.css" type="text/css" rel="stylesheet" />
   			</c:if>
   			<c:if test="${temp.label==10 }">
   			<link href="${ctxStatic}/admin/css/models/ami-pro5.css" type="text/css" rel="stylesheet" />
   			</c:if>
   			<c:if test="${temp.label==19 }">
   			<link href="${ctxStatic}/admin/css/models/ami-pro3.css" type="text/css" rel="stylesheet" />
   			</c:if>
      	</c:when>
		<c:otherwise>
			<div class="container">
				<div class="setting-container preview-container">
					<div class="h5_info">
						...
						<div class="info_title">
								<div class="left logo">
									<img alt="" src="${fns:getUploadUrl()}${companyLogo}">
								</div>
				...
				.ready(
						function() {
							...
							$
									.ajax({
										type : "get",
										url : "${ctx}/weixin/weixinsdk?url="
												+ encodeURIComponent(localurl),
										dataType : "json",
										success : function(data) {
											shareLink = "https://open.weixin.qq.com/connect/oauth2/authorize?appid="
													+ data.appId
													+ "&redirect_uri="
