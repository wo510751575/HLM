		...
		$(document).ready(function() {
			...
			if($("#btnBatchAllot").css("background-color")!='rgb(128, 128, 128)'){//如果不是灰色,开启点击事件
				$("#btnBatchAllot").click(function(){
					...
					// 正常打开
					top.$.jBox.open("iframe:${ctx}/member/addFriends/view?noWxGm="+noWxGm,"导购列表", 520, 400,{//宽高
						id:9527,
						draggable: true,
						showClose: true,
						buttons:{},		//去除按钮
						...
						//,ajaxData:{"noWxGm":noWxGm,"vals":vals}
...
<div class="container">
	...
	<ul class="nav nav-tabs">
		<li class="active"><a href="${ctx}/member/addFriends/">未分配客户</a></li>
	</ul>


	<form id="searchForm" action="${ctx}/member/addFriends/" method="post" class="breadcrumb form-search">
		<input id="pageNo" name="pageNo" type="hidden" value="${page.pageNo}"/>
		<input id="pageSize" name="pageSize" type="hidden" value="${page.pageSize}"/>
		<ul class="ul-form">
		    <li><label>客户微信：</label>
		    	<input type="text" name="noWx" value="${findAddFriendsAllotPage.noWx}" class="input-medium" maxlength="100" placeholder="客户微信">
			</li>
		<%-- 	<li><label>微信号：</label>
				<input type="text" name="noWx" value="${findAddFriendsAllotPage.noWx}" class="input-medium" maxlength="100" placeholder="微信号">
			</li> --%>
			<li><label>手机号：</label>
			...
			<li>
				...
				value="<fmt:formatDate value="${findAddFriendsAllotPage.startTime}" pattern="yyyy-MM-dd"/>"
				...
				value="<fmt:formatDate value="${findAddFriendsAllotPage.endTime}" pattern="yyyy-MM-dd"/>"
					onclick="WdatePicker({dateFmt:'yyyy-MM-dd',isShowClear:true});"
					onfocus="WdatePicker({minDate:'#F{$dp.$D(\'beginDate\')}',maxDate:'2120-10-01'})"
						/>&nbsp;&nbsp;
			</li>
...
<%-- 				<shiro:hasPermission name="member:addFriends:edit"><th>操作</th></shiro:hasPermission> --%>
		...
		<c:forEach items="${page.list}" var="addFriends" varStatus="number">
			<tr>
				...
				<td>
					${not empty addFriends.alias?addFriends.alias:addFriends.noWx}
				</td>
				<td>
					${addFriends.nickName}
				</td>
				...
				<td>
					<c:if test="${fn:toUpperCase(addFriends.sex)=='MALE'}">男</c:if>
					<c:if test="${fn:toUpperCase(addFriends.sex)=='FEMALE'}">女</c:if>
					<c:if test="${fn:toUpperCase(addFriends.sex)=='UNKNOWN'}">未知</c:if>
				</td>
				<td>
					${addFriends.noWxGm}
				</td>
