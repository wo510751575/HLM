...
<html>
<head>
    <title>${not empty data.code?'服务类型修改':'服务类型添加'}</title>
    ...
    $(document).ready(function() {
        ...
        $("#projectCode").change(function(){
    		$("#serviceName").val($(this).find("option:selected").text());
        });
        $("#projectCode").change();
...
<div class="container">
    <ul class="nav nav-tabs">
        <li><a href="${ctx}/member/serviceTypeChoose">服务类型选择列表</a></li>
	    <li class="active"><a href="${ctx}/member/serviceTypeChoose/form?code=${data.code}">${not empty data.code?'服务类型选择修改':'服务类型选择添加'}</a></li> 
    </ul>
     <tags:message content="${message}"/>
     <form id="inputForm" action="${ctx}/member/serviceTypeChoose/${not empty data.code?'edit':'save'}" method="post" enctype="multipart/form-data" class="form-horizontal">
        <input id="code" name="code" type="hidden" value="${data.code}"/>
        <input id="shopName" name="shopName" type="hidden" value="${data.shopName}"/>
         <input id="serviceName" name="serviceName" type="hidden" value="${data.serviceName}"/>
        <div id="base_div" class="tab_div">   
            ...
            <div class="control-group">
             ...
             <div class="control-group">
                ...
                <div class="controls">
                 <div class="form-group form-group-select2">
					<select name="projectCode" id="projectCode"  style="width:220px">
			 			<c:forEach items="${list}" var="item">
			 			<option value="${item.code}"<c:if test="${item.code eq data.serviceCode}">selected="selected"</c:if>>${item.projectName}</option>
			 			</c:forEach>
					</select>
					</div>
                </div>
             ...
             <div class="control-group">
                ...
                <div class="controls">
                 <div class="form-group form-group-select2">
					<select name="serviceType" id="serviceType"  style="width:220px">
			 			<c:forEach items="${serviceTypes}" var="item">
			 			<option value="${item}"<c:if test="${item eq data.serviceType}">selected="selected"</c:if>>${item.name}</option>
			 			</c:forEach>
					</select>
					</div>
                </div>
