...
public class MemberLoginInfoServiceImpl implements IMemberLoginInfoService { 
	...
	public DelMemberLoginInfoReturn delMemberLoginInfo(
		...
		try {
			...
			logger.debug("delMemberLoginInfo(DelMemberLoginInfo) - end - return value={}", delMemberLoginInfoReturn); 
	...
	public UpdateMemberLoginInfoReturn updateMemberLoginInfo(
		...
		logger.debug("updateMemberLoginInfo(UpdateMemberLoginInfo updateMemberLoginInfo={}) - start", updateMemberLoginInfo); 
		...
		try {
			...
			logger.debug("updateMemberLoginInfo(UpdateMemberLoginInfo) - end - return value={}", updateMemberLoginInfoReturn); 
	...
	public FindMemberLoginInfoReturn findMemberLoginInfo(
		...
		logger.debug("findMemberLoginInfo(FindMemberLoginInfo findMemberLoginInfo={}) - start", findMemberLoginInfo); 
		...
		try {
			...
			logger.debug("findMemberLoginInfo(FindMemberLoginInfo) - end - return value={}", findMemberLoginInfoReturn); 
	...
	public Page<FindMemberLoginInfoPageReturn> findMemberLoginInfoPage(
			...
			throws TsfaServiceException {
		logger.debug("findMemberLoginInfoPage(FindMemberLoginInfoPage findMemberLoginInfoPage={}) - start", findMemberLoginInfoPage); 
		...
		logger.debug("findMemberLoginInfoPage(FindMemberLoginInfoPage) - end - return value={}", returnPage); 
		return  returnPage;
	}
	 ...
	 */
	@Override
	public FindMemberLoginInfoReturn findLastMemberLoginInfo(String memberNo) throws TsfaServiceException {
		logger.debug("findLastMemberLoginInfo(String memberNo={}) - start", memberNo); 

		AssertUtils.notNullAndEmpty(memberNo,"会员编号不能为空");
		try {
			FindMemberLoginInfoReturn memberLoginInfo = memberLoginInfoDao.findLastMemberLoginInfo(memberNo);
			
			// 会员最后登录终端的mac（ios客户端登录时该字段为极光推送注册的registerId）
			if(memberLoginInfo != null && StringUtils.isNotEmpty(memberLoginInfo.getMac())) {
				// 获取缓存中mac对应最后登录的导购编号
				String memberNoByMac = redisCache.hget(MemberConstants.MEMBER_LOGIN_SMS_ID_KEY, memberLoginInfo.getMac());
				// 与当前导购编号不同，即表示mac终端最后登录的导购不是当前导购memberNo
				if(!memberNo.equals(memberNoByMac)) {
					logger.info("MAC[{}]对应终端最后登录的导购{}，不是当前导购{}", memberLoginInfo.getMac(), memberNoByMac, memberNo);
					memberLoginInfo.setMac(null);
				}
			}
			
			logger.debug("findLastMemberLoginInfo(String) - end - return value={}", memberLoginInfo); 
			return memberLoginInfo;
		} catch (Exception e) {
			logger.error("查询会员最后一条登录信息错误！",e);
			throw new TsfaServiceException(ErrorCode.MEMBER_LOGIN_INFO_FIND_ERROR,"查询会员最后一条登录信息错误！",e);
		}
	} 
